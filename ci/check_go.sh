...
check_live_python_oracles() {
  ...
  if ! (
    ...
    "${GO_ENV_OFF[@]}" \
      ...
      go test -mod=readonly -count=1 \
        -run '^(TestFernetCipherMatchesLivePythonCustomSalt|TestFernetCipherMatchesLivePythonDefaultSalt|TestFernetRefusesWithoutKeyLikePython|TestCredentialFieldReadsMatchLivePython)$' \
        ./internal/providerfoundation/...
  ); then
    rm -rf -- "${proof_dir}"
    return 1
  fi
  for proof_name in providerfoundation-credentials providerfoundation-credentials-default-salt providerfoundation-credentials-no-key providerfoundation-credential-field-reads; do
  ...
  if ! (
    ...
    "${GO_ENV_OFF[@]}" \
      ...
      PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
      go test -mod=readonly -count=1 \
        -run '^(TestParseDatetimeMatchesLivePydantic|TestParseDateMatchesLivePydantic|TestFromISOFormatMatchesLivePython|TestPydanticMatchesLivePydanticDumpJSON)$' \
        ./internal/api/pytime
  ); then
    rm -rf -- "${proof_dir}"
    return 1
  ...
  printf 'go test -count=1: internal/pythonparity (str repr and utf-8 replace vs live Python)\n'
  if ! (
    cd "${ROOT}"
    "${GO_ENV_OFF[@]}" \
      GOWORK=off \
      ...
      PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
      go test -mod=readonly -count=1 \
        -run '^(TestStrReprMatchesLivePython|TestDecodeUTF8ReplaceMatchesLivePython|TestURLSplitMatchesLivePython|TestIsoformatUTCMatchesLivePython|TestFnMatchMatchesLivePython|TestSequenceRatioMatchesLivePython|TestSanitizeErrorTextMatchesLivePython)$' \
  ...
  printf 'go test -count=1: internal/cli (dho root flags before the command vs the REAL dev-hops root parser)\n'
  if ! (
    cd "${ROOT}"
    "${GO_ENV_OFF[@]}" \
      GOWORK=off \
      ...
      DEV_HEALTH_LIVE_PYTHON_ORACLE_PROOF_DIR="${proof_dir}" \
      PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
      go test -mod=readonly -count=1 -run '^TestRootFlagsMatchLivePython$' ./internal/cli
  ); then
    rm -rf -- "${proof_dir}"
    return 1
  fi
  proof_file="${proof_dir}/cli-root-flags"
  if [ ! -f "${proof_file}" ] || [ "$(cat "${proof_file}")" != "executed" ]; then
    printf 'ERROR: the dho root flags live Python oracle measurement did not occur\n' >&2
    rm -rf -- "${proof_dir}"
    return 1
  fi
