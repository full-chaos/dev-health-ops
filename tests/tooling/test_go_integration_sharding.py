...
def test_shard_plan_is_exhaustive_nonempty_and_machine_readable(
    ...
    assert len(expected_provider_tests) == 930
    assert len(expected_integration_tests) == 109
    ...
    assert len(provider_flattened) == len(set(provider_flattened)) == 930
...
def test_each_shard_dry_run_executes_only_its_manifest_assignment() -> None:
    ...
    assert len(selected_tests) == len(set(selected_tests)) == 930
