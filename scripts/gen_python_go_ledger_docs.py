...
KIND_LEDGER: dict[str, dict[str, str]] = {
    ...
    "sync.team_autoimport": {
        ...
        "state": "mixed (migration-state below) — native for linear/github/gitlab (CHAOS-4431/4432/4434, Done), bridge for jira only (CHAOS-4198, not yet ported)",
        "ticket": "CHAOS-4198 (jira populator, remaining scope) + CHAOS-4435 (delete kind/routes/modules once jira is native — blocked by CHAOS-4194→CHAOS-4193 dependency) + CHAOS-4492 (this ledger regen)",
    },
...
BRIDGE_ROUTE_LEDGER: dict[str, dict[str, str]] = {
    ...
    "/api/internal/worker-sync/reference-discovery-populate": {
        "go_caller": "`bridge.go:133 PopulateReferenceDiscovery`, called from `bridge_discovery_executor.go:45`, wired into `NativeReferenceDiscoveryService`'s executor — LIVE, synchronous, blocking. As of #1989/#1984/#1985 the caller passes `exclude_providers` for linear/github/gitlab (routed to native Go collectors instead, see `sync.team_autoimport` kind row)",
        "python_handler": "`worker_sync.py:286 reference_discovery_populate_reference` -> `reference_discovery.py:226` -> `team_autoimport.py:169 run_team_autoimport_strict` -> per-provider `populate()` — now reached for jira only",
        "computes": "jira teams/members/memberships/project-ownership rows; credential decrypt + PagerDuty OAuth rotation Python-side. linear/github/gitlab excluded via `exclude_providers`",
        "state": "bridge — live for jira; dead for linear/github/gitlab pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native, blocked) + CHAOS-4492 (this ledger regen)",
    },
    "/api/internal/worker-sync/team-autoimport": {
        "go_caller": "`bridge.go:113 TeamAutoImport`, called from `RegisterTeamAutoimportWorker` (`worker.go:93`), wired `cmd/dev-health-worker/sync_dispatch.go:565` — LIVE, fire-and-forget. Same `exclude_providers` split as reference-discovery-populate for linear/github/gitlab",
        "python_handler": "`worker_sync.py:269 team_autoimport_reference` -> `team_autoimport.py:228 run_post_sync_team_autoimport` -> same per-provider `populate()` — now reached for jira only",
        "computes": "same tables as reference-discovery-populate, best-effort variant, jira only",
        "state": "bridge — live for jira; dead for linear/github/gitlab pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native, blocked) + CHAOS-4492 (this ledger regen)",
    },
}
...
WORKER_FILE_LEDGER: dict[str, dict[str, str]] = {
    ...
    "team_autoimport_github.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4434 (`950752653`, Done) — Go-native collector now writes GitHub teams/team_memberships in prod-bound code, and the bridge request excludes github via `exclude_providers`, so `populate()` here is dead-in-practice (import-reachable, not execution-reachable) pending file deletion",
        "ticket": "CHAOS-4435 (delete once jira is native; blocked)",
    },
    "team_autoimport_gitlab.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4432 (`5bff38a5a`, Done) — Go-native collector now writes GitLab teams/team_memberships/team_project_ownership, and the bridge request excludes gitlab via `exclude_providers`, so `populate()` here is dead-in-practice pending file deletion",
        "ticket": "CHAOS-4435 (delete once jira is native; blocked)",
    },
    "team_autoimport_jira.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; sole live writer of Jira team_project_ownership — the only provider still reaching `populate()` through the bridge; not yet split into its own porting child",
        "ticket": "CHAOS-4198 (not yet split into a Jira child)",
    },
    "team_autoimport_linear.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4431 (`27bef7286`, Done) — the previously-unwired Go route (CHAOS-3716) is now registered in `nativeTeamCatalogCollectors` and writes Linear teams/team_memberships/team_project_ownership, and the bridge request excludes linear via `exclude_providers`, so `populate()` here is dead-in-practice pending file deletion",
        "ticket": "CHAOS-4435 (delete once jira is native; blocked)",
    },
    "team_autoimport.py": {
        "category": "a",
