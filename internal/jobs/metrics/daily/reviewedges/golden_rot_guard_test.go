...
// TestReviewEdgesGoldenMatchesLivePython is the rot guard for
// tests/fixtures/daily_review_edges_python_golden.json (CHAOS-4279),
...
// Python (compute_review_edges) and checked in;
...
// compute_review_edges changes its numbers out from under the frozen file.
func TestReviewEdgesGoldenMatchesLivePython(t *testing.T) {
	...
	repoRoot := reviewEdgesRepositoryRoot(t)
	python := reviewEdgesLivePython(t, repoRoot)
	generator := filepath.Join(
		repoRoot, "tests", "fixtures", "generate_daily_review_edges_python_golden.py",
	)
	...
	goldenPath := filepath.Join(
		repoRoot, "tests", "fixtures", "daily_review_edges_python_golden.json",
	)
	...
	if string(frozen) == string(rendered) {
		...
		if writeErr := os.WriteFile(
			filepath.Join(proofDirectory, "review-edges-golden"), []byte("executed"), 0o644,
		); writeErr != nil {
	...
	t.Errorf(
		"live Python no longer reproduces the frozen golden.\n" +
			"This is Python drift, not a Go bug: tests/fixtures/daily_review_edges_python_golden.json " +
			"was generated from production Python and frozen. Regenerate with\n" +
			"    python tests/fixtures/generate_daily_review_edges_python_golden.py\n" +
...
func reviewEdgesRepositoryRoot(t *testing.T) string {
	t.Helper()
	return repositoryRoot(t)
}

// reviewEdgesLivePython mirrors cicd's cicdLivePython: PYTHON env var wins,
...
// golden.
func reviewEdgesLivePython(t *testing.T, repoRoot string) string {
	t.Helper()
