...
func drilldownIssuesSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(drilldownIssuesEnabledEnvVar)); enabled {
...
func loadDrilldownIssuesRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildDrilldownIssuesRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(drilldownIssuesSwitchFromEnv(getenv))
