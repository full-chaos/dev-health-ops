...
//
//  1. no /api/v1/* route file under internal/queryapi/server may answer an error
...
// non-test .go file directly under internal/queryapi/server for the two violation
...
func TestDataUnavailableCallSitesLogTheCause(t *testing.T) {
	...
	if !sawDataUnavailableCall {
		t.Fatal("found zero logging-wrapper call sites in the query-api server package (internal/queryapi/server) -- this guard's premise (there are 503 degradation sites to check) no longer holds; investigate before trusting a green result")
	}
