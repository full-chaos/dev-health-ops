...
| [`docs/harness-design.md`](docs/harness-design.md) | **Deliverable 3.** Shadow projection/query design, the four arms, compose+kind placement, and the gated bring-up plan. Design only — nothing is brought up. |
...
Current state: **204 passed, 136 skipped** (the skips are fault×oracle pairs
...
2. **Every guard observed failing.** `tests/test_oracle_fault_modes.py` pairs
   ...
   Verified by planting three defects in the oracle engine; all three were
   killed in the right guards (see `docs/harness-design.md` §4).
3. **A measurement that did not happen FAILS.** There is no skip path in the
   ...
   recorded `NOT_RUN` rather than an aborted sweep, and `TrialReport` marks any
   aggregate containing an unmeasured oracle as NOT COMPARABLE.
