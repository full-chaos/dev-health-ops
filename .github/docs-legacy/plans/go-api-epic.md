...
**Wave 0 also includes: extract the shared Go data/auth library from `acr`** — store/contract layer only (ClickHouse client + org-scoping/reader-dedup primitives, `devhealthschema`'s table contracts, `devhealthfacts`'s fact readers, and `internal/auth`'s principal-envelope verifier), explicitly excluding resolver/response shapes. This must land **before or alongside** the rest of Wave 0, since `query-api`'s first reachable route (used to prove the switch-gated-reachability test above) should already be built on the extracted readers, not on hand-rolled ClickHouse queries that would need porting again later. `acr` migrates its own imports onto the extracted module in the same change (no behavior change to `acr`'s existing responses); `ops`'s native Go metrics readers (CHAOS-3092) migrate onto it separately, later, not as part of this wave.
...
flowchart LR
  ...
  query -->|fact reads via shared lib| sharedlib[shared Go data/auth library: devhealthschema + devhealthfacts + principal-envelope verifier, extracted from acr Wave 0]
  acr[acr: Go Context Fabric / Ask Dev] -->|fact reads via shared lib, same extraction| sharedlib
  ...
  acr -->|direct reads, unchanged outside the shared-lib extraction| pg
```
