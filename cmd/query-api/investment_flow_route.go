...
func investmentFlowSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(investmentFlowEnabledEnvVar)); enabled {
...
// Postgres.
func loadInvestmentFlowRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
// package follows.
func buildInvestmentFlowRoute() (flowHandler, repoTeamHandler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadInvestmentFlowRouteConfig()
	if !cfgOK {
