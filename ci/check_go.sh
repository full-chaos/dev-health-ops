...
check_live_python_oracles() {
  ...
  if ! (
    ...
    "${GO_ENV_OFF[@]}" \
      ...
      PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
      go test -mod=readonly -count=1 \
        -run '^(TestQueryIntMatchesLivePydantic|TestQueryBoolMatchesLivePydantic)$' \
        ./internal/api/pybody
  ); then
    rm -rf -- "${proof_dir}"
    return 1
  ...
  for proof_name in api-policy-principal api-pyjson api-pyjson-dumps api-pyjson-model api-orgs-registry api-pytime api-pytime-date api-health-revisions api-pybody-queryint api-pybody-querybool pythonparity-strrepr pythonparity-utf8-replace httpapi-forwarded-scheme api-customerpush-schema api-customerpush-bodies; do
