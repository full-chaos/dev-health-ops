...
| `metrics.daily_finalize` | River worker `cmd/dev-health-worker/daily.go:236` (`daily.NewFinalizeHandler(store, compatibility)`) | follows partition completion (same run) | none found | Go run bookkeeping (`postgres.go:630,661,1100,1126,1160`); finalize-side per-family compute still holds a `compatibility` (bridge) executor dependency | `public.daily_metrics_runs` | argued — code read, not re-executed this session | mixed (migration-state: `go_default`/`river`) | CHAOS-3092 children (per-family) |
...
| `feature_flag_sync.py` | CELERY-TASK-ONLY / DEAD | no decorator, ZERO importers anywhere (workers/, api/, tests/) — more orphaned than a celery-task file | CHAOS-4439 (dead worker modules) |
...
| `org_guard.py` | LIBRARY / SHARED | imported only by metrics_daily.py/sync_scheduler.py (both dead) — no surviving live consumer | CHAOS-4439 (dead worker modules, flagged alongside) |
...
| `report_task.py` | CELERY-TASK-ONLY / DEAD | `@celery_app.task` (L113 execute_saved_report); sole importer tasks.py:12 | CHAOS-4439 (dead worker modules) |
...
| `system_tasks.py` | LIBRARY / SHARED | pure re-export shim of system_ops/system_webhooks; sole importer tasks.py:23-27 — dead aggregator, real functions separately live via direct imports | CHAOS-4439 (dead worker modules, flagged alongside) |
...
- **CHAOS-4439** -- 10+ dead Celery-only Python worker modules with no live caller (`external_ingest_recompute.py`, `external_ingest_reconciler.py`, `metrics_daily.py`, `metrics_extra.py`, `queue_monitor.py`, `report_task.py`, `sync_reconciler.py`, `sync_scheduler.py`, `team_drift_sync.py`, `tasks.py`, plus orphans `feature_flag_sync.py`/`org_guard.py`/`system_tasks.py`) -- safe to delete now that Celery is archived (CHAOS-4026).
