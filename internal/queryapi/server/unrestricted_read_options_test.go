...
// the fast, container-free half that runs in `go test ./internal/queryapi/server/...`
...
// non-test .go file directly under internal/queryapi/server for a call to
...
//
// Scope: only the server package's own files, not its _test.go
...
func TestEveryClickHouseReadClientUsesTheSharedUnrestrictedOptions(t *testing.T) {
	...
	if !sawAnyCallSite {
		t.Fatalf("found zero %s call sites in the query-api server package (internal/queryapi/server) -- this guard's premise (there are read clients to check) no longer holds; investigate before trusting a green result", targetCall)
	}
