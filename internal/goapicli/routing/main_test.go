...
// A refusal (a state the operator must resolve) exits 3; a crash exits 1.
// A calling script has to be able to tell them apart. Before the S1 fold
// (dho goapi routing) a refusal exited 2, the same code `flag` itself uses
// for a usage error; 3 (cli.ExitRefused) gives it a code of its own.
...
func TestAnUnclassifiedErrorIsARefusalAndOnlyErrInternalCrashes(t *testing.T) {
	...
	if got := exitCodeFor(refuse("nope")); got != 3 {
		t.Fatalf("exitCodeFor(refusal) = %d, want 3", got)
	}
	if got := exitCodeFor(fmt.Errorf("wrapped: %w", refuse("nope"))); got != 3 {
	...
	// three times.
	if got := exitCodeFor(errors.New("something nobody classified")); got != 3 {
...
// to be able to tell "I would not do that" (3) from "I broke" (1), and
// the split existed but this path did not go through it.
func TestAnUnknownVerbRefusesWithExitThree(t *testing.T) {
	err := run([]string{"definitely-not-a-verb"})
	if err == nil {
		t.Fatal("an unknown verb must be an error")
	}
	if got := exitCodeFor(err); got != 3 {
		t.Fatalf("exit code = %d, want 3 -- an unknown verb is a refusal, and the Python CLI sets that convention", got)
	}
...
func TestEveryVerbRefusesItsOwnMissingPreconditions(t *testing.T) {
	...
	for name, testCase := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if got := exitCodeFor(err); got != 3 {
				t.Fatalf("run(%v) exits %d, want 3 -- every one of these is a state the operator resolves", testCase.argv, got)
			}
		})
	}
}
...
func TestEnableAndRepointRefuseWithNoQueryAPIURLConfiguredAtAll(t *testing.T) {
	...
	for name, argv := range map[string][]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if got := exitCodeFor(err); got != 3 {
				t.Fatalf("run(%v) exits %d, want 3", argv, got)
			}
		})
	}
}
...
func TestConnectPostgresRefusesAndNeverEchoesTheDSN(t *testing.T) {
	...
	for name, malformed := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if got := exitCodeFor(err); got != 3 {
				t.Fatalf("a malformed DSN exits %d, want 3 -- it is the operator's to fix", got)
			}
...
func TestCredentialBearingURLsAreRefusedBeforeTheyCanBePrinted(t *testing.T) {
	...
	for _, raw := range []string{
		...
		if got := exitCodeFor(err); got != 3 {
			t.Fatalf("a credential-bearing URL exits %d, want 3", got)
		}
	}
...
func TestUnparseableAndNonHTTPURLsAreRefusedWithoutEchoingThem(t *testing.T) {
	...
	for name, raw := range map[string]string{
		...
		if got := exitCodeFor(err); got != 3 {
			t.Fatalf("%s exits %d, want 3", name, got)
		}
	}
}
...
func TestEveryVerbRefusesATimeoutThatDisablesItsOwnBound(t *testing.T) {
	...
	for _, timeout := range refused {
		...
		if exitCodeFor(err) != 3 {
			t.Fatalf("-timeout %s exited %d, want 3 -- an unusable flag is the operator's to fix", timeout, exitCodeFor(err))
		}
...
func TestEveryVerbRefusesAnUnconsumedArgument(t *testing.T) {
	...
	for name, argv := range map[string][]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if exitCodeFor(err) != 3 {
				t.Fatalf("%v exited %d, want 3 -- a mistyped command line is the operator's to fix", argv, exitCodeFor(err))
			}
