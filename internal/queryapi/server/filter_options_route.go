...
func filterOptionsSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(filterOptionsEnabledEnvVar)); enabled {
...
func loadFilterOptionsRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildFilterOptionsRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadFilterOptionsRouteConfig(getenv)
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(filterOptionsSwitchFromEnv(getenv))
