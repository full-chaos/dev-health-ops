...
//     EVERY non-test .go file under internal/queryapi for a composite
...
func parseQueryRouteDocuments(t *testing.T, repoRoot string) (documentConstants map[string]string, operationToIdentifier map[string]string) {
	t.Helper()
	path := filepath.Join(repoRoot, "internal", "queryapi", "server", "query_route.go")
...
// root/internal/queryapi (excluding models_gen.go/generated.go, which only
...
func buildPopulatabilityOracle(t *testing.T, repoRoot string, typeNames map[string]bool) *populatabilityOracle {
	...
	fset := token.NewFileSet()
	// All of query-api's code lives under internal/queryapi: the server
	// package and its packages (cmd/query-api holds only the thin main).
	// Each root must yield files, or the oracle would silently shrink.
	...
	for _, sel := range selections {
		for _, f := range sel.fields {
			...
			if oracle.instanceCount(f.typeName, info.goName) == 0 {
				violations = append(violations, violation{
					operation: sel.operation, field: f,
					reason: fmt.Sprintf("model.%s.%s (json %q) is never assigned in any composite literal anywhere under internal/queryapi -- nothing populates it", f.typeName, info.goName, f.fieldName),
				})
				continue
			}
