...
def test_positive_two_independent_at_risk_dimensions_qualify_team_needs_attention() -> (
    ...
    result = _evaluate_with_registry(
        _AUTHORIZED_TEST_REGISTRY, observations, org_id="org-1"
    )
...
def test_negative_single_provisional_threshold_alone_produces_no_launch_finding() -> (
    ...
    result = evaluate_registry(observations, org_id="org-1")
...
def test_negative_measured_zero_and_no_data_are_not_collapsed() -> None:
    ...
    result_zero = evaluate_registry({rule_id: measured_zero}, org_id="org-1")
    zero_finding = (
    ...
    result_no_data = evaluate_registry({rule_id: no_data}, org_id="org-1")
    no_data_finding = (
