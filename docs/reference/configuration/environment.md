...
The Go worker fleet's five DSNs (`POSTGRES_URI`, `WORKER_DATABASE_URI`, `COORDINATOR_DATABASE_URI`, `CLICKHOUSE_URI`, `MIGRATION_DATABASE_URI`) also accept a component form -- discrete host/port/user/password/db environment variables (`DEV_HEALTH_PG_DOMAIN_*`, `DEV_HEALTH_PG_QUEUE_*`, `DEV_HEALTH_PG_COORDINATOR_*`, `DEV_HEALTH_CH_*`, `DEV_HEALTH_MIGRATION_PG_*`) that are assembled into the DSN safely, so a password containing `#`, `@`, `/`, or `:` never needs manual URL-encoding. The inline/`_FILE` form and the component form are mutually exclusive per DSN, refused if both are set; see each binary's `--help` and `internal/platform/config`'s `ResolveDSN` for the exact keys. Components are opaque strings: the assembled DSN is validated by round-tripping it through the driver that will consume it (`pgconn.ParseConfig`, `clickhouse.ParseDSN`), so a component is refused only where that driver refuses it, and the driver's own message is reported under the names of the settings that produced it. Give an IPv6 host in bare form (`::1`, not `[::1]`) -- the assembler brackets it. The one rule beyond the driver's own: the component form's `*_HOST` variable must resolve to exactly ONE endpoint, checked on the driver's parsed configuration, so a comma-separated multi-host list is refused (both drivers read a comma there as a failover-host delimiter, not a literal character). A deployment that needs multiple failover hosts for a DSN must use the inline pre-built-URI form instead, where both drivers' native multi-host syntax is unaffected.
...
!!! note "Go workers are configured by flags first (CHAOS-4020)"
    ...
    The Go worker processes (`dev-health-worker`, `dho reconciler`,
    `dho scheduler`, `dho stream-runner`) resolve every setting
