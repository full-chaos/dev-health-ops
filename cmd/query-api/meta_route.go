...
func metaSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(metaEnabledEnvVar)); enabled {
...
func loadMetaRouteConfig() (clickHouseURI string, ok bool) {
...
func buildMetaRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, cfgOK := loadMetaRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(metaSwitchFromEnv())
