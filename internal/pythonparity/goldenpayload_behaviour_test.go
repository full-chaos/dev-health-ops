...
// syntheticGuard builds a throwaway registry entry for the cases below, which
// exercise comparePayload itself against hand-written documents rather than
// shipped fixtures. It is NOT a bypass of the registry: this file is not a
// *_rot_guard_test.go, so TestEveryRotGuardUsesTheRegistry does not police it,
// and a real guard cannot reach this helper -- it can only name a registry
// entry.
func syntheticGuard(fields ...string) payloadGuard {
	return payloadGuard{fixture: "synthetic-document", fields: fields}
}
...
func TestPayloadComparisonIgnoresProvenanceAndCatchesData(t *testing.T) {
  ...
  "cases": [{"name": "a", "evidence_json": "[{\"value\": 1.5}]"}]
	...
	t.Run("a bumped interpreter version PASSES", func(t *testing.T) {
		...
		if err := comparePayload([]byte(frozen), []byte(rendered), syntheticGuard("cases", "distinct_input_values")); err != nil {
	...
	t.Run("a flipped case FAILS", func(t *testing.T) {
		...
		err := comparePayload([]byte(frozen), []byte(rendered), syntheticGuard("cases", "distinct_input_values"))
		if err == nil {
	...
	t.Run("a RENAMED payload field FAILS rather than passing vacuously", func(t *testing.T) {
		...
		err := comparePayload([]byte(renamed), []byte(renamed), syntheticGuard("cases", "distinct_input_values"))
		if err == nil {
...
func TestPayloadMismatchIsAttributed(t *testing.T) {
  ...
  "cases": [{"name": "a", "value": %s}]
	...
	t.Run("same interpreter says the DATA moved", func(t *testing.T) {
		...
		err := comparePayload([]byte(frozen), []byte(rendered), syntheticGuard("cases"))
		if err == nil {
	...
	t.Run("a document with no interpreter is FATAL, not tolerated", func(t *testing.T) {
		...
		err := comparePayload([]byte(frozen), []byte(stripped), syntheticGuard("cases"))
