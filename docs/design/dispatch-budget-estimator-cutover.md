...
- Cutover = a second implementation of that interface in Go, a wrapper that runs both and compares, then a per-provider config switch. The gate code does not change.
...
| credential_id, decrypted credentials | `resolve_run_auth` (sync_bootstrap.py:163): run-stamped credential first, then integration credential; Fernet decrypt | every estimator, ONLY for bucket identity |
...
- `shadow`: `shadowBudgetEstimator` calls the Python bridge (authoritative, returned to the gate) AND `goBudgetEstimator`; compares per unit; logs and counts; the gate sees ONLY Python's answer. Go errors and panics are swallowed and counted, never reach the gate.
...
| Database read error for the whole chunk | ERROR the pass (fail closed). Today's analogue (5xx) is fail open. This is a deliberate change; the gate runs in one transaction and a failed read poisons it anyway. Overturnable default. |
...
### 3.6 Production facts (read-only, from `_records/6036-prod-budget-facts.md`, rev 123, one pod ~1 h) and what they change
...
- Consequence for the shadow phase: it proves ESTIMATE parity (it compares estimates, not decisions), which stays valid on an idle gate. It cannot prove DEFERRAL parity, because production never exercises the defer path. Replacement check before each provider switch: a deferral-parity harness runs the real `enforceRun`, `observeRun` and `activeBudgetConsumption` over a seeded Postgres (testcontainer) with deliberately tiny limits, once per estimator (Python-recorded estimates replayed from the shadow log, then `goBudgetEstimator`), and asserts identical admitted and deferred unit sets, `nextDeferredAt`, and per-bucket consumption. The cells come from the shadow corpus (real datasets, spans, flags); limits are set so every bucket defers at least once.
...
| B | Estimate input loader (I/O only): unit row, dataset options, org-wide placeholder, run-stamped-then-integration credential resolve, Fernet decrypt to typed JSON. First task: verify domain-role grants | M | A | Postgres testcontainer; resolved (credential_id, host, fingerprint) equals Python `resolve_run_auth` on seeded rows incl. NULL-stamped runs |
...
| D1 | `goBudgetEstimator` adapter: loader + estimators behind the `budgetEstimator` interface, per-unit degrade-to-empty, NOT wired anywhere | M | B, C1..C4 (per provider) | fake-loader unit tests for every failure class; oracle end-to-end per provider |
...
| D3 | Shadow comparator: run Go beside Python, compare per unit, counters and diff log, swallow Go errors and panics | M | D2 | every diff class; Go panic and Go slowness cannot alter the returned Python answer |
...
1. Fingerprint byte parity is the top risk. A wrong fingerprint does not fail a test in production: it silently splits one bucket into two, so limits stop binding. Mitigation: PR A oracle corpus; shadow class `bucket_differs` is a stop signal.
