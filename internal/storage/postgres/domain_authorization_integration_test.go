...
func TestDomainAuthorizationRequiresExactCanaryAndReconcilerPrivileges(t *testing.T) {
	...
	for _, statement := range []string{
		...
		"CREATE TABLE public.scheduled_jobs (id uuid PRIMARY KEY, org_id uuid, sync_config_id uuid, job_type text, status text, schedule_cron text, next_run_at timestamptz, created_at timestamptz, updated_at timestamptz)",
		...
		"GRANT SELECT (id, org_id, sync_config_id, job_type, status, schedule_cron, next_run_at, created_at), UPDATE (next_run_at, updated_at) ON TABLE public.scheduled_jobs TO " + authorizedDomainRole,
