...
Every Go worker, scheduler, reconciler, and stream-runner process serves these on `:8080/metrics` (`internal/platform/health/server.go`). Nothing in this repository configures a Prometheus scrape target for that endpoint, in any deployment topology (Compose or Helm) — that wiring is the operator's responsibility, same as the rest of this repo's monitoring stack (CHAOS-3985). The `alerts/rules.yml` `go_workers` rule group assumes a scrape job named `dev-health-go-(worker|scheduler|reconciler|stream).*` — whatever scrape mechanism you configure must assign job names matching that pattern, or the rules will silently never match. The Helm chart names a `metrics` container port (8080) on every go-worker pod, ready for a scrape config or ServiceMonitor to target; Compose service names do not currently carry the `dev-health-go-` prefix and need an explicit `job_name` override wherever they're scraped from.
