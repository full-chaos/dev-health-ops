...
func investmentExplainSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(investmentExplainEnabledEnvVar)); enabled {
...
func loadInvestmentExplainRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience, registryPostgresURI string, ok bool) {
	cfg, queryOk := loadQueryRouteConfig(getenv)
	if !queryOk {
		return "", "", "", "", "", false
	}
...
func buildInvestmentExplainRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	decryptor, decryptorErr := providerfoundation.NewFernetDecryptor(
		secrets.NewValue(getenv("SETTINGS_ENCRYPTION_KEY")), getenv("SETTINGS_ENCRYPTION_SALT"))
	if decryptorErr != nil {
		log.Printf(
	...
	routeMux := routeswitch.NewMux(investmentExplainSwitchFromEnv(getenv))
...
// and the work-unit query filter against -- both read FROM %s AS
// work_unit_investments (investmentexplain's reader.go/workunitreader.go),
