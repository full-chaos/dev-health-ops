...
check_live_python_oracles() {
  ...
  printf 'go test -count=1: internal/providersync (--search batch repository listing vs the REAL Python listing, CHAOS-6684)\n'
  if ! (
    cd "${ROOT}"
    "${GO_ENV_OFF[@]}" \
      GOWORK=off \
      ...
      DEV_HEALTH_LIVE_PYTHON_ORACLE_PROOF_DIR="${proof_dir}" \
      PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
      go test -mod=readonly -count=1 -run '^TestRepoListingMatchesLivePython$' ./internal/providersync
  ); then
    rm -rf -- "${proof_dir}"
    return 1
  fi
