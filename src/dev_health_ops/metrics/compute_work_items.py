...
# (WAIT_STATUSES, _calculate_flow_breakdown, GroupBucket, UserBucket) and
# compute_work_item_team_attributions are DELETED -- native Go executor
# (daily-partition path) and providersync's own ingest-time derivation are
# the only producers of work_item_metrics_daily/work_item_user_metrics_
# daily/work_item_cycle_times/work_item_team_attributions now. Their last
# remaining Python callers (job_daily.py, job_work_items.py's run_work_
# items_sync_job, and the providersync oracle_pairs scripts) are all gone
# too -- see the two prior commits on this branch. Frozen JSON snapshots of
# their last live comparisons live in internal/providersync/testdata/
# oracle_frozen/.
