...
async def materialize_investments(config: MaterializeConfig) -> dict[str, int]:
    ...
    try:
        ...
        repo_ids = _resolve_repo_ids(sink, config.repo_ids, config.team_ids, config_org_id=config.org_id or "")
        edges = fetch_work_graph_edges(sink, repo_ids=repo_ids, org_id=config.org_id or "")
        ...
        work_items = fetch_work_items(sink, work_item_ids=issue_ids, org_id=config.org_id or "")
        active_hours = fetch_work_item_active_hours(sink, work_item_ids=issue_ids, org_id=config.org_id or "")
        repo_prs = _group_prs_by_repo(pr_ids)
        prs = fetch_pull_requests(sink, repo_numbers=repo_prs, org_id=config.org_id or "")
        repo_commits = _group_commits_by_repo(commit_ids)
        commits = fetch_commits(sink, repo_commits=repo_commits, org_id=config.org_id or "")
        commit_churn = fetch_commit_churn(sink, repo_commits=repo_commits, org_id=config.org_id or "")
        ...
        parent_titles = fetch_parent_titles(sink, work_item_ids=parent_ids, org_id=config.org_id or "")
        epic_titles = fetch_parent_titles(sink, work_item_ids=epic_ids, org_id=config.org_id or "")
