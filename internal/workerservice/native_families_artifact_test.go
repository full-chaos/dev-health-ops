...
// parseDailyGoFile parses internal/workerservice/daily.go once per call.
...
func TestNativeFamiliesArtifactUpToDate(t *testing.T) {
	...
	if err != nil {
		t.Fatalf(
			"reading %s: %v -- run UPDATE_NATIVE_FAMILIES_ARTIFACT=1 go test ./internal/workerservice/... -run TestNativeFamiliesArtifactUpToDate to create it",
			path, err,
		)
	}
	if string(actual) != string(expected) {
		t.Fatalf(
			"contracts/native-families/v1/native-families.json is stale relative to internal/workerservice/daily.go's "+
...
func TestNativeFamiliesArtifactMatchesKnownSplit(t *testing.T) {
	...
	// CHAOS-4291: complexity was the last remaining-metrics family on the
	// compat bridge; wantRemainingCompat is empty now that it cut over.
	var wantRemainingCompat []string
...
// extractWorkgraphExecutors statically parses internal/workerservice/workgraph.go's
