...
def test_shard_plan_is_exhaustive_nonempty_and_machine_readable(
    ...
    #
    # Codex review then added 3 more (981 -> 984): the chunked route's own
    # fail-closed assertion for a blocking archive-bounds issue, which the
    # retargeted oracle test covered only on the non-chunked twin, plus the
    # totality pair -- a walk whose artifacts are ALL unreadable fails with its
    # own cause, while a partially unreadable walk still completes.
    #
    ...
    # providersync count.)
    assert len(expected_provider_tests) == 984
    ...
    assert len(provider_flattened) == len(set(provider_flattened)) == 984
...
def test_each_shard_dry_run_executes_only_its_manifest_assignment() -> None:
    ...
    assert len(selected_tests) == len(set(selected_tests)) == 984
