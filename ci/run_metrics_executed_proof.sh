...
(
  ...
  # This harness runs the go-built binary directly against the checked-out
  # source tree, with no /app -- point the override at the real repo-relative
  # file so construction doesn't refuse the whole "metrics" queue's coverage
  # (ValidateStartup requires every selected-queue kind to have a registered
  # handler; an unavailable config file is a refusal, not a skip, same as a
  # missing ClickHouse schema).
