...
func newResolveInTxFixture(t *testing.T, ctx context.Context) (*pgxpool.Pool, *Controller, func()) {
	t.Helper()
	instance, err := containers.StartPostgres(ctx)
	if err != nil {
		t.Fatal(err)
	}
	// Explicit, generous MaxConns: the fencing test below holds THREE
	// connections concurrently (the producer's open transaction, the
	// blocked pause attempt, and the pg_stat_activity poll checking on it)
	// -- pgxpool's default sizing must not be trusted to leave headroom for
	// a fourth caller, or the poll itself could starve waiting for a free
	// connection and look identical to a genuinely blocked pause.
	poolConfig, err := pgxpool.ParseConfig(instance.URI)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `
		...
		INSERT INTO public.worker_job_routes
			(job_kind, transport, paused, generation, updated_at)
		VALUES ('sync.provider_unit', 'river', FALSE, 1, statement_timestamp())`); err != nil {
		t.Fatal(err)
	}
	controller, err := NewController(pool, integrationRegistry{jobruntime.Descriptor{
		Kind: "sync.provider_unit", Route: "river", RollbackRoute: "celery",
	}}, idleQuiescer{})
	if err != nil {
		t.Fatal(err)
	}
	...
	return pool, controller, cleanup
}

// TestResolveInTxReturnsTheCurrentTransportForAHealthyRoute pins the
// straightforward case: an unpaused, undrifted route resolves to its
// checked-in transport, same as Resolve.
func TestResolveInTxReturnsTheCurrentTransportForAHealthyRoute(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	pool, controller, cleanup := newResolveInTxFixture(t, ctx)
	defer cleanup()
	...
	transport, err := controller.ResolveInTx(ctx, tx, "sync.provider_unit")
	if err != nil {
		t.Fatalf("ResolveInTx() error = %v, want nil", err)
	}
	if transport != "river" {
		t.Fatalf("ResolveInTx() transport = %q, want %q", transport, "river")
	}
}

// TestResolveInTxRejectsAnAlreadyPausedRoute pins the base case a producer's
// enqueue must never slip through: a route paused before the producer's
// transaction even begins must be observed as paused, not silently
// forwarded as its last-known transport.
func TestResolveInTxRejectsAnAlreadyPausedRoute(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	pool, controller, cleanup := newResolveInTxFixture(t, ctx)
	defer cleanup()

	if _, err := pool.Exec(ctx, `UPDATE public.worker_job_routes SET paused = TRUE WHERE job_kind = $1`, "sync.provider_unit"); err != nil {
		t.Fatal(err)
	}
	...
	defer func() { _ = tx.Rollback(ctx) }()
	if _, err := controller.ResolveInTx(ctx, tx, "sync.provider_unit"); !errors.Is(err, ErrPaused) {
		t.Fatalf("ResolveInTx() error = %v, want %v", err, ErrPaused)
	}
}

// TestResolveInTxFencesAConcurrentPauseUntilTheProducerCommits is the
// scenario Python's resolve_worker_job_route comment names directly: "A
// rollback [pause] takes FOR UPDATE on the same row, so it cannot report
// quiescence while a producer that observed the old route can still stage
// work after the route change commits." A producer that calls ResolveInTx
// inside its own transaction and then stages its own outbox write in that
// SAME transaction must make a concurrent pause attempt BLOCK until the
// producer's transaction commits -- proving the fence is real, not just
// that the two operations happen to be ordered correctly by luck. Without
// ResolveInTx's FOR UPDATE (i.e. using Resolve's plain pool-based read
// instead), the pause could commit while the producer's transaction is
// still open, and the producer would go on to stage work under a route an
// operator already believed was quiesced.
func TestResolveInTxFencesAConcurrentPauseUntilTheProducerCommits(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	pool, controller, cleanup := newResolveInTxFixture(t, ctx)
	defer cleanup()
	...
	// A pgx.Tx that is never committed nor rolled back holds its connection
	// (and any row locks) forever -- including through an early t.Fatal
	// unwinding this goroutine. Without this, a failure anywhere below would
	// leave the pause goroutine permanently blocked on the still-held lock,
	// and the deferred pool.Close() in cleanup() would itself hang waiting
	// for that connection to be released, wedging the whole test binary.
	// Rollback after a successful Commit is a documented no-op error, safe
	// to ignore unconditionally.
	defer func() { _ = producerTx.Rollback(ctx) }()
	transport, err := controller.ResolveInTx(ctx, producerTx, "sync.provider_unit")
	if err != nil {
		t.Fatalf("ResolveInTx() error = %v, want nil", err)
	}
	if transport != "river" {
	...
	go func() {
		_, pauseErr := pool.Exec(ctx, `
			UPDATE public.worker_job_routes SET paused = TRUE, generation = generation + 1, updated_at = statement_timestamp()
			WHERE job_kind = $1`, "sync.provider_unit")
		pauseResult <- pauseErr
	}()
	waitForBlockedPauseUpdate(t, ctx, pool)

	// The pause is provably blocked right now (waitForBlockedRouteUpdate
	// only returns once Postgres itself reports a lock-waiter). Only after
	// the producer commits may it proceed.
	select {
	case err := <-pauseResult:
		t.Fatalf("pause completed before the producer's transaction committed (err=%v) -- the fence did not hold", err)
	default:
	}
	...
	if err := <-pauseResult; err != nil {
		t.Fatalf("pause after commit: %v", err)
	}
	...
	if !state.Paused {
		t.Fatal("route not paused after the concurrent UPDATE committed")
	}
}
...
// waitForBlockedPauseUpdate is waitForBlockedRouteUpdate's sibling for a
// plain UPDATE waiter (an operator pause) rather than a locking SELECT --
// the query text pg_stat_activity reports for the two never overlaps, so
// reusing the SELECT...FOR UPDATE pattern here would never match and burn
// the full poll deadline on every run.
func waitForBlockedPauseUpdate(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	waitForLockWaiter(t, ctx, pool, "%UPDATE public.worker_job_routes SET paused%", "pause update never blocked on the producer's row lock")
}
