...
BRIDGE_ROUTE_LEDGER: dict[str, dict[str, str]] = {
    ...
    "/api/internal/worker-sync/reference-discovery-populate": {
        "go_caller": "`bridge.go:133 PopulateReferenceDiscovery`, called from `bridge_discovery_executor.go:45` (`BridgeDiscoveryExecutor`), wired as the `Fallback` of `TeamCatalogDiscoveryExecutor` (`sync_dispatch.go:576`) — LIVE, synchronous, blocking. As of #1989/#1984/#1985 `TeamCatalogDiscoveryExecutor.Discover` only reaches this Fallback for a sync run whose OWN provider has no entry in `nativeTeamCatalogCollectors` (jira today); linear/github/gitlab sync runs never call it at all. Corrected 2026-08-29 (CHAOS-4492 codex round 1, P2): the call itself carries no provider-exclusion field -- `BridgeDiscoveryExecutor.Discover` explicitly ignores its own `provider` argument (see its doc comment) and sends only `{organization_id, sync_run_id}`; Python resolves the provider itself from the sync run's own integration row",
        "python_handler": "`worker_sync.py:286 reference_discovery_populate_reference` -> `reference_discovery.py:226` -> `team_autoimport.py:169 run_team_autoimport_strict` -> per-provider `populate()` — reached for jira via this sync-time route only",
        "computes": "jira teams/members/memberships/project-ownership rows via this route; credential decrypt + PagerDuty OAuth rotation Python-side. linear/github/gitlab sync runs never reach this handler (Go never calls it for them), but the SAME Python function is also reachable outside this bridge route -- see `sync.team_autoimport` kind row's backfill note",
        "state": "bridge — live for jira sync-time dispatch; dead for linear/github/gitlab SYNC-TIME dispatch pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven. NOT dead as a code path: the backfill tool calls the underlying Python function directly, bypassing this HTTP route",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native AND backfill re-pointed, blocked) + CHAOS-4492 (this ledger regen)",
    },
    "/api/internal/worker-sync/team-autoimport": {
        "go_caller": "`bridge.go:113 TeamAutoImport`, called from `teamCatalogAutoimportBridge.TeamAutoImport` (`cmd/dev-health-worker/team_catalog_clients.go:426`), the wrapper `RegisterTeamAutoimportWorker` (`worker.go:93`) registers, wired `sync_dispatch.go:565` — LIVE, fire-and-forget. Same per-sync-run provider-routing split as reference-discovery-populate: `teamCatalogAutoimportBridge` resolves the run's own provider and calls its embedded `CoordinatorBridge.TeamAutoImport` (i.e. this route) only when that provider is absent from `nativeTeamCatalogCollectors` (jira today); it never sends a provider list either -- `teamAutoImportReference` (`bridge.go:92-96`) is `{organization_id, sync_run_id}` only",
        "python_handler": "`worker_sync.py:269 team_autoimport_reference` -> `team_autoimport.py:228 run_post_sync_team_autoimport` -> same per-provider `populate()` — reached for jira via this sync-time route only",
        "computes": "same tables as reference-discovery-populate, best-effort variant, jira sync-time dispatch only",
        "state": "bridge — live for jira sync-time dispatch; dead for linear/github/gitlab SYNC-TIME dispatch pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven. NOT dead as a code path: the backfill tool calls the underlying Python function directly, bypassing this HTTP route",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native AND backfill re-pointed, blocked) + CHAOS-4492 (this ledger regen)",
    },
}
...
WORKER_FILE_LEDGER: dict[str, dict[str, str]] = {
    ...
    "team_autoimport_github.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4434 (`950752653`, Done) — a GitHub sync run's post-sync/reference-discovery dispatch now resolves to the Go-native collector in `nativeTeamCatalogCollectors` and never calls the bridge, so `populate()` is no longer reached via sync-time dispatch. Corrected 2026-08-29 (CHAOS-4492 codex round 1, P2): still LIVE, not dead-in-practice -- `src/dev_health_ops/backfill/runner.py:101 _run_strict_reference_discovery_for_backfill` calls `run_team_autoimport_strict(provider=...)` directly for whatever provider an operator backfills, bypassing the Go routing entirely, so a GitHub backfill still executes this file's `populate()`",
        "ticket": "CHAOS-4435 (delete once jira is native AND the backfill path is re-pointed at the native collectors; blocked)",
    },
    "team_autoimport_gitlab.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4432 (`5bff38a5a`, Done) — a GitLab sync run's post-sync/reference-discovery dispatch now resolves to the Go-native collector and never calls the bridge, so `populate()` is no longer reached via sync-time dispatch. Corrected 2026-08-29 (CHAOS-4492 codex round 1, P2): still LIVE, not dead-in-practice -- the backfill path (`backfill/runner.py:101`) calls `run_team_autoimport_strict(provider='gitlab', ...)` directly, bypassing Go routing, so a GitLab backfill still executes this file's `populate()`",
        "ticket": "CHAOS-4435 (delete once jira is native AND the backfill path is re-pointed at the native collectors; blocked)",
    },
    "team_autoimport_jira.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; sole live writer of Jira team_project_ownership — the only provider whose sync-time dispatch still reaches `populate()` through the bridge (`nativeTeamCatalogCollectors` has no jira entry); also reachable via the backfill path like the other three providers; not yet split into its own porting child",
        "ticket": "CHAOS-4198 (not yet split into a Jira child)",
    },
    "team_autoimport_linear.py": {
        "category": "a",
        "evidence": "imported by team_autoimport.py; ported CHAOS-4431 (`27bef7286`, Done) — the previously-unwired Go route (CHAOS-3716) is now registered in `nativeTeamCatalogCollectors`, so a Linear sync run's post-sync/reference-discovery dispatch never calls the bridge. Corrected 2026-08-29 (CHAOS-4492 codex round 1, P2): still LIVE, not dead-in-practice -- the backfill path (`backfill/runner.py:101`) calls `run_team_autoimport_strict(provider='linear', ...)` directly, bypassing Go routing, so a Linear backfill still executes this file's `populate()`",
        "ticket": "CHAOS-4435 (delete once jira is native AND the backfill path is re-pointed at the native collectors; blocked)",
    },
    "team_autoimport.py": {
        "category": "a",
