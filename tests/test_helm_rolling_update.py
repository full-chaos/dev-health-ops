...
def _strategies(docs: list[dict]) -> dict[str, dict]:
    return {
        ...
        and (
            ...
            or "go-worker" in d["metadata"]["labels"].get("app.kubernetes.io/component", "")
        )
    }
...
def test_query_api_rolling_update_is_values_driven() -> None:
    strategies = _strategies(_render(json_sets={"queryApi.rollingUpdate": '{"maxUnavailable":1,"maxSurge":0}'}))
    assert strategies["t-dev-health-query-api"] == _TIGHT
    assert all(s == _DEFAULT for n, s in strategies.items() if not n.endswith("query-api"))
...
def test_go_workers_default_and_group_override() -> None:
    ...
    workers = {n: s for n, s in _strategies(docs).items() if not n.endswith("query-api")}
...
def test_group_level_wins_over_the_workers_default() -> None:
    values = yaml.safe_load(
        run(["helm", "show", "values", str(_CHART)], check=True, capture_output=True, text=True).stdout
    )
    ...
    override.write_text(yaml.safe_dump({"goWorkers": {"rollingUpdate": _TIGHT, "groups": groups}}))
    try:
        out = run(
            ["helm", "template", "t", str(_CHART), "-f", str(override)], check=True, capture_output=True, text=True
        ).stdout
    finally:
    ...
    by_name = {
        ...
        if d and d["kind"] == "Deployment" and d["metadata"]["labels"].get("app.kubernetes.io/component") == "go-worker"
    }
...
def test_schema_rejects_an_unknown_rolling_update_key() -> None:
    with pytest.raises(CalledProcessError):
        run(
            ["helm", "template", "t", str(_CHART), "--set-json", 'goWorkers.rollingUpdate={"nonsense":1}'],
            check=True,
            capture_output=True,
            text=True,
        )
