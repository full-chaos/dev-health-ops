...
Routing configuration must match deployed consumers. Enabling provider-specific or cost-class queues before workers consume them can strand jobs. Checked-in Go deployment profiles remain disabled with zero minimum replicas even though the sync-dispatch contract targets River; deploy the consumers and apply the durable routes before sending work.
