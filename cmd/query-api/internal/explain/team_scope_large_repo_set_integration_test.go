...
// only be shown against the genuine driver.
package explain
...
// (1.88k-2.22k distinct rows observed there for the failing requests).
...
// connection (for seeding) and a QueryClient built with the SAME default
// options production uses (no MaxResultRows override), matching
// teamattribution_integration_test.go's own copy of this pattern.
...
// seedTeamScopedRepos writes n repos rows plus one user_metrics_daily row
// per repo, all under the same org/team, giving the team exactly n
// DISTINCT matching repo ids -- the shape resolveRepoIDsForTeams'
// original query (and teamRepoScopeCondition's own nested one) both
// select over.
func seedTeamScopedRepos(ctx context.Context, t *testing.T, admin stdclickhouse.Conn, orgID, teamID string, n int) []string {
	t.Helper()
...
// TestLargeTeamRepoScope_OldMaterializedQueryHitsResultRowCap reproduces
// the underlying defect directly: the EXACT statement text the prior
// resolveRepoIDsForTeams sent (a standalone SELECT DISTINCT of every
// matching repo_id) against a team whose true matching count exceeds the
// read-only client's default row ceiling. This must fail with ClickHouse's
// own code-396 "Limit for result exceeded" -- the same failure production's
// own query-api-503-causes.txt telemetry recorded for these requests --
...
// all.
func TestLargeTeamRepoScope_OldMaterializedQueryHitsResultRowCap(t *testing.T) {
	ctx := context.Background()
	admin, client := newExplainTestClickHouse(ctx, t)

	const orgID = "org-scale-old"
	const teamID = "team-scale-old"
	seedTeamScopedRepos(ctx, t, admin, orgID, teamID, teamScopeLargeRepoCount)
...
// TestLargeTeamRepoScope_ExplainRouteSucceeds is this ticket's own proof:
// the SAME oversized team scope, driven through the real route entry point
// (BuildExplainResponse, exactly as the HTTP handler calls it), against
// the SAME default-options client the reproduction above used, must
// succeed -- the pushed-down team condition never asks the client to
// materialize the team's own repo-id set as a standalone result.
func TestLargeTeamRepoScope_ExplainRouteSucceeds(t *testing.T) {
	ctx := context.Background()
	admin, client := newExplainTestClickHouse(ctx, t)

	const orgID = "org-scale-new"
	const teamID = "team-scale-new"
	seedTeamScopedRepos(ctx, t, admin, orgID, teamID, teamScopeLargeRepoCount)
	...
	if err != nil {
		t.Fatalf("BuildExplainResponse with a %d-repo team scope: %v (this is the exact request shape production's query-api-503-causes.txt recorded degrading to 503)", teamScopeLargeRepoCount, err)
	}
	if got == nil {
