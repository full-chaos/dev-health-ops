...
NOTE ON THE ID: this is 0088 and its parent is 0087. It was authored against
0085, because main independently took 0086 (dev_run_qua_shadow) and 0087
(dev_qua_shadow_budget_reservations) while this lane was in flight and neither
existed on the feature branch yet -- pointing down_revision at 0087 before they
arrived was a hard KeyError at ScriptDirectory load, verified rather than
assumed. So 0088 reserved the id and parked its parent at 0085, with the
obligation recorded here that the sync bringing main's 0086/0087 across MUST
re-point it.

That sync is the merge this file now sits on, and the re-point is done:
down_revision is 0087, so application_schema@head resolves to 0088 and the
branch heads are exactly {0066, 0088}. Had it been missed the failure would
have been loud rather than silent -- three heads, and
get_revision("application_schema@head") raising CommandError, which all five
migration-head pin tests exercise directly. The obligation is left described
rather than deleted so the next lane that has to reserve an id ahead of main
can see the shape that worked.
"""
