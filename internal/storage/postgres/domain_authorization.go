...
func coordinatorPosture() RolePosture {
	return RolePosture{
		RequiredTables: []TablePrivilege{
			...
			// CHAOS-5459 repair verbs (internal/jobs/repair), run by workerctl on
			// this operator role. The *_repairs audit tables are INSERT-only: a
			// repair is appended, never rewritten. daily_metrics_partitions is
			// read for the claim-liveness check. The three ledger/request tables
			// below are column-scoped (see ColumnScoped) because a relation
			// cannot be both table-wide and column-scoped.
			{"work_graph_execution_repairs", true, false, false},
			{"metric_compatibility_execution_repairs", true, false, false},
			{"daily_metrics_partitions", false, false, false},
		},
		ColumnScoped: []ColumnPrivilege{
			// CHAOS-5459 repair verbs. work_graph_execution_requests keeps the
			// SELECT+INSERT the publisher already had (every column, unchanged in
			// effect) and gains UPDATE on exactly the two columns the repair sets.
			// The ledger and metric execution tables get SELECT on only the
			// columns the repair reads and UPDATE on only the columns it sets. The
			// FOR UPDATE row locks need UPDATE on at least one column per table.
			{"work_graph_execution_requests", "id", "SELECT"},
			{"work_graph_execution_requests", "id", "INSERT"},
			{"work_graph_execution_requests", "org_id", "SELECT"},
			{"work_graph_execution_requests", "org_id", "INSERT"},
			{"work_graph_execution_requests", "kind", "SELECT"},
			{"work_graph_execution_requests", "kind", "INSERT"},
			{"work_graph_execution_requests", "scope", "SELECT"},
			{"work_graph_execution_requests", "scope", "INSERT"},
			{"work_graph_execution_requests", "model_ref", "SELECT"},
			{"work_graph_execution_requests", "model_ref", "INSERT"},
			{"work_graph_execution_requests", "prompt_ref", "SELECT"},
			{"work_graph_execution_requests", "prompt_ref", "INSERT"},
			{"work_graph_execution_requests", "llm_concurrency", "SELECT"},
			{"work_graph_execution_requests", "llm_concurrency", "INSERT"},
			{"work_graph_execution_requests", "spend_limit_microunits", "SELECT"},
			{"work_graph_execution_requests", "spend_limit_microunits", "INSERT"},
			{"work_graph_execution_requests", "correlation_id", "SELECT"},
			{"work_graph_execution_requests", "correlation_id", "INSERT"},
			{"work_graph_execution_requests", "idempotency_key", "SELECT"},
			{"work_graph_execution_requests", "idempotency_key", "INSERT"},
			{"work_graph_execution_requests", "state", "SELECT"},
			{"work_graph_execution_requests", "state", "INSERT"},
			{"work_graph_execution_requests", "claim_token", "SELECT"},
			{"work_graph_execution_requests", "claim_token", "INSERT"},
			{"work_graph_execution_requests", "lease_expires_at", "SELECT"},
			{"work_graph_execution_requests", "lease_expires_at", "INSERT"},
			{"work_graph_execution_requests", "attempt_count", "SELECT"},
			{"work_graph_execution_requests", "attempt_count", "INSERT"},
			{"work_graph_execution_requests", "created_at", "SELECT"},
			{"work_graph_execution_requests", "created_at", "INSERT"},
			{"work_graph_execution_requests", "updated_at", "SELECT"},
			{"work_graph_execution_requests", "updated_at", "INSERT"},
			{"work_graph_execution_requests", "state", "UPDATE"},
			{"work_graph_execution_requests", "updated_at", "UPDATE"},
			{"work_graph_execution_ledger", "request_id", "SELECT"},
			{"work_graph_execution_ledger", "state", "SELECT"},
			{"work_graph_execution_ledger", "attempt_count", "SELECT"},
			{"work_graph_execution_ledger", "state", "UPDATE"},
			{"work_graph_execution_ledger", "output_evidence", "UPDATE"},
			{"work_graph_execution_ledger", "failure_detail", "UPDATE"},
			{"work_graph_execution_ledger", "completed_at", "UPDATE"},
			{"metric_compatibility_executions", "id", "SELECT"},
			{"metric_compatibility_executions", "worker_kind", "SELECT"},
			{"metric_compatibility_executions", "operation", "SELECT"},
			{"metric_compatibility_executions", "run_id", "SELECT"},
			{"metric_compatibility_executions", "partition_id", "SELECT"},
			{"metric_compatibility_executions", "claim_token", "SELECT"},
			{"metric_compatibility_executions", "state", "SELECT"},
			{"metric_compatibility_executions", "attempt_count", "SELECT"},
			{"metric_compatibility_executions", "state", "UPDATE"},
			{"metric_compatibility_executions", "output_evidence", "UPDATE"},
			{"metric_compatibility_executions", "completed_at", "UPDATE"},
			{"metric_compatibility_executions", "last_attempt_at", "UPDATE"},
