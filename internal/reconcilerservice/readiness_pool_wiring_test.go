...
func TestDomainReadinessChecksRunOnTheReadinessPoolNotTheWorkPool(t *testing.T) {
	...
	for name, run := range map[string]func(*postgresReconcilerDatabase, context.Context) error{
		"DomainReady": func(d *postgresReconcilerDatabase, ctx context.Context) error { return d.DomainReady(ctx) },
		...
		t.Run(name, func(t *testing.T) {
			database := &postgresReconcilerDatabase{pools: newPools(), domainRole: "devhealth_domain", riverSchema: "river"}
