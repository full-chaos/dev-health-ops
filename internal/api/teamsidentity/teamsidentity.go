...
func Routes(conn driver.Conn, guard *policy.Guard, logger *slog.Logger, pool *pgxpool.Pool, decryptor providerfoundation.CredentialDecryptor) []httpapi.Route {
	...
	return []httpapi.Route{
		...
		{Method: http.MethodGet, Pattern: "/api/v1/admin/teams/{team_id}/discover-members",
			Handler: guard.Wrap(policy.AdminOrg, http.HandlerFunc(h.discoverMembers))},
		{Method: http.MethodGet, Pattern: "/api/v1/admin/teams/{team_id}/infer-members",
			Handler: guard.Wrap(policy.AdminOrg, http.HandlerFunc(h.inferMembers))},
		{Method: http.MethodPost, Pattern: "/api/v1/admin/teams/{team_id}/confirm-members",
			Handler: h.decodeFirst(guard, http.HandlerFunc(h.confirmMembers))},
		{Method: http.MethodPost, Pattern: "/api/v1/admin/teams/{team_id}/confirm-inferred-members",
			Handler: h.decodeFirst(guard, http.HandlerFunc(h.confirmInferredMembers))},
