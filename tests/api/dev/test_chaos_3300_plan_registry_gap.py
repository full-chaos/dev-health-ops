...
-- a genuine capability downgrade with no operational signal. This tests the
...
async def test_plan_registry_gap_is_loud_for_a_normally_plan_governed_intent(
    ...
    still complete via the legacy fallback (no behavior change) but must
    also emit the WARNING log record and increment the counter -- silence
