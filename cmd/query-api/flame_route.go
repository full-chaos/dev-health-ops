...
import (
	...
	"github.com/full-chaos/dev-health-ops/cmd/query-api/internal/flame"
...
func flameSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(flameEnabledEnvVar)); enabled {
...
func loadFlameRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildFlameRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadFlameRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(flameSwitchFromEnv())
	...
	entryHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		claims, ok := authenticateRESTRequest(w, r, verifier, "flame")
		if !ok {
			return
		}
