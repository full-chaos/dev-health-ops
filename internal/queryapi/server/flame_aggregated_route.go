...
func flameAggregatedSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(flameAggregatedEnabledEnvVar)); enabled {
...
// Postgres.
func loadFlameAggregatedRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildFlameAggregatedRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadFlameAggregatedRouteConfig(getenv)
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(flameAggregatedSwitchFromEnv(getenv))
