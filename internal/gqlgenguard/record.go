...
func renderRecord(res *Result, plan *Plan, gen Generator, moduleRoot, copyRoot *os.Root) string {
	...
	b.WriteString("#          (status \"same\" carries no hashes: tree == generated by definition, and a\n")
	b.WriteString("#          hash there only made every unrelated change to the file conflict)\n")
	drifted := make([]Change, 0, len(res.Changes))
	for _, c := range res.Changes {
		status := statusOf(c)
		if status == "same" {
			// tree == generated: the status already says everything the two
			// equal hashes would. Leaving them out keeps this line stable
			// while a hand-edited-but-regenerable file (schema.resolvers.go,
			// whose bodies gqlgen preserves) changes, so parallel edits to it
			// do not all conflict on this record.
			fmt.Fprintf(&b, "digest same %s\n", c.Path)
		} else {
...
func digestLines(record string) map[string]string {
	...
	for line := range strings.SplitSeq(record, "\n") {
		...
		// digest <status> <path> tree=<d> generated=<d>, or the hash-less
		// digest same <path>.
		hashless := len(fields) == 3 && fields[1] == "same"
		if len(fields) != 5 && !hashless {
			continue
		}
