...
func TestWorkerReadinessChecksDoNotQueueBehindTheQueueWorkPool(t *testing.T) {
	...
	newWorkPool := func() *pgxpool.Pool {
		...
		config.MaxConns = 2 // the worker's queue_control and coordinator sizes
	...
	database := &postgresWorkerDatabase{
		...
		queueRole: "postgres", domainRole: "postgres", riverSchema: "river",
	}
	...
	awaitAnswered(t, &database.queueProbe, &database.riverSchemaProbe)
	hold(queue)
	hold(coordinator)
...
// CHAOS-6934: the GITHUB_PROJECTS_V2 startup census is a readiness input (it runs on the probe path
// whenever the environment variable is set), so it reads on the domain readiness pool: with the work
// pool unusable it still answers.
func TestGitHubProjectsV2CensusReadsOnTheReadinessPool(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 4*time.Minute)
	defer cancel()
	instance, err := containers.StartPostgres(ctx)
	if err != nil {
	...
	probe, err := pgxpool.New(ctx, instance.URI)
	if err != nil {
		t.Fatal(err)
	}
	...
	work.Close() // every acquire on the work pool now fails at once
	database := &postgresWorkerDatabase{pools: &postgres.RuntimePools{Domain: work, DomainProbe: probe}}
	if _, err := database.GitHubProjectsV2Configured(ctx); err != nil {
		t.Fatalf("the census failed with a usable readiness pool and an unusable work pool: %v (it reads on the work pool)", err)
	}
}
