...
func TestGqlgenConfigParserInputDomain(t *testing.T) {
	cases := []domainCase{
		...
		// Every row below names no usable schema. gqlgen ACCEPTS all of them --
		// measured: its StringList turns a null into an empty list and a bare
		// scalar into a one-element list that globs to nothing -- and then
		// generates an API from the built-in prelude alone, which `generate`
		// would copy over the checked-in outputs. The guard refuses them.
		// (Round 1's version of these rows asserted ACCEPT, and one comment
		// claimed a null decodes to the default; both encoded gqlgen's
		// behaviour as the contract instead of the guard's.)
		...
		{field: "schema", shape: "out of vocabulary (a glob that matches nothing)", config: strings.Replace(domainBase, "schema: [schema.graphql]", "schema: [nothing/*.graphql]", 1),
			wantErr: "match no file"},
		{field: "schema", shape: "wrong container type (a map)", config: strings.Replace(domainBase, "schema: [schema.graphql]", "schema: {a: b}", 1),
			wantErr: "load gqlgen config"},
...
// The provenance shapes gqlgen v0.17.66 writes, typed out once for the tables
// below. They are NOT trusted on their own: the knob matrix
// (assertRealGenerationStaysInsideThePlan) regenerates with the real CLI and
// fails if gqlgen ever writes a shape the guard does not accept.
const (
	headerLine      = "// Code generated by github.com/99designs/gqlgen, DO NOT EDIT."
	stubNotice      = "// This file will not be regenerated automatically.\n//\n// It serves as dependency injection for your app, add any dependencies you require here.\n"
	followNoticeNoV = "// This file will be automatically regenerated based on the schema, any resolver implementations\n// will be copied through when generating and any unknown code will be moved to the end.\n// Code generated by github.com/99designs/gqlgen\n"
	followNotice    = "// This file will be automatically regenerated based on the schema, any resolver implementations\n// will be copied through when generating and any unknown code will be moved to the end.\n// Code generated by github.com/99designs/gqlgen version v0.17.66\n"
...
// stubResolverOfLength returns a resolver file carrying the dependency-injection
// notice whose notice block ends at exactly n bytes, by lengthening the package
// name. It is how the boundary rows put the END of the anchored shape on the
// scan window's edge.
func stubResolverOfLength(t *testing.T, n int) string {
	t.Helper()
	fixed := len("package p\n\n") + len(stubNotice)
	if n < fixed {
		t.Fatalf("cannot build a %d-byte stub resolver; the minimum is %d", n, fixed)
	}
	body := "package p" + strings.Repeat("x", n-fixed) + "\n\n" + stubNotice
	if len(body) != n {
		t.Fatalf("stub resolver is %d bytes, want %d", len(body), n)
	}
	return body
}
...
// of that path in the working tree, and which configuration section declared
// it.
//
// Provenance is ANCHORED: gqlgen writes its notices at fixed positions in fixed
// shapes, so the guard accepts exactly those and nothing that merely CONTAINS
// the words. Round 1 found the old substring scan accepting a hand-written file
// whose comment happened to quote the marker; the first rows below are that
// class, one row per place the words can sit without being gqlgen's header.
func TestProvenanceGuardInputDomain(t *testing.T) {
	const (
		execOut     = "gen/generated.go" // declared by exec
		resolverOut = "gen/resolver.go"  // declared by resolver
	)
	cases := []struct {
		shape string
		// target is the declared output the cell plants; body is its content.
		target string
		body   string
		...
		{shape: "exec: canonical (the generated header at line 1)", target: execOut, body: headerLine + "\n\npackage gen\n"},
		{shape: "exec: round 1's repro (the words in a hand-written comment below the package clause)", target: execOut,
			body:    "package gen\n\n// Code generated by github.com/99designs/gqlgen appears in a copied example.\nconst Human = \"hand-written\"\n",
			wantErr: "treated as hand-written"},
		{shape: "exec: the exact header line MID-FILE", target: execOut,
			body: "package gen\n\nfunc A() {}\n\n" + headerLine + "\n", wantErr: "treated as hand-written"},
		{shape: "exec: the exact header inside a STRING LITERAL", target: execOut,
			body: "package gen\n\nconst s = \"" + headerLine + "\"\n", wantErr: "treated as hand-written"},
		{shape: "exec: the exact header in a COMMENT AFTER CODE on one line", target: execOut,
			body: "var x = 1 " + headerLine + "\n\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "exec: the header at line 2 (boundary+1 on position)", target: execOut,
			body: "\n" + headerLine + "\n\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "exec: the header at line 1 with CRLF line endings", target: execOut,
			body: headerLine + "\r\n\r\npackage gen\r\n", wantErr: "CRLF"},
		{shape: "exec: the header at line 1 behind a UTF-8 byte-order mark", target: execOut,
			body: "\xef\xbb\xbf" + headerLine + "\n\npackage gen\n", wantErr: "byte-order mark"},
		{shape: "exec: the header with a trailing space", target: execOut,
			body: headerLine + " \n\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "exec: the header with NO terminating newline (the whole file)", target: execOut,
			body: headerLine, wantErr: "treated as hand-written"},
		{shape: "exec: a near-miss header (no comma)", target: execOut,
			body: "// Code generated by github.com/99designs/gqlgen DO NOT EDIT.\n\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "exec: another generator's header (out of vocabulary)", target: execOut,
			body: "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "exec: a RESOLVER notice on an exec output (a shape gqlgen never writes there)", target: execOut,
			body: "package gen\n\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "exec: zero (an empty file)", target: execOut, body: "", wantErr: "treated as hand-written"},
		{shape: "exec: out of vocabulary (a hand-written file)", target: execOut,
			body: "package gen\n\n// a person wrote this\n", wantErr: "treated as hand-written"},
		{shape: "exec: wrong type (a directory)", target: execOut, plant: func(t *testing.T, full string) {
			if err := os.Remove(full); err != nil {
				t.Fatalf("remove: %v", err)
			}
		...
		{shape: "exec: wrong type (a symbolic link)", target: execOut, plant: func(t *testing.T, full string) {
			if err := os.Remove(full); err != nil {
				t.Fatalf("remove: %v", err)
			}
		...
		// ---- resolver output: a notice directly below `package <name>` + one blank line ----
		{shape: "resolver: the single-file notice", target: resolverOut, body: "package gen\n\n" + singleNotice},
		{shape: "resolver: the preserved single-file notice", target: resolverOut, body: "package gen\n\n" + preservedNotice},
		{shape: "resolver: the follow-schema notice with its version", target: resolverOut, body: "package gen\n\n" + followNotice},
		{shape: "resolver: the follow-schema notice without its version (omit_gqlgen_version_in_file_notice)", target: resolverOut,
			body: "package gen\n\n" + followNoticeNoV},
		{shape: "resolver: the dependency-injection notice", target: resolverOut, body: "package gen\n\n" + stubNotice},
		{shape: "resolver: the follow-schema notice TRUNCATED (its first line cut short)", target: resolverOut,
			body: "package gen\n\n// This file will be automatically regenerated based on the schema\n", wantErr: "treated as hand-written"},
		{shape: "resolver: the dependency-injection notice with its LAST line reworded", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(stubNotice, "add any dependencies you require here.", "written by a person.", 1), wantErr: "treated as hand-written"},
		{shape: "resolver: the follow-schema notice with its SECOND line reworded", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(followNotice, "any unknown code will be moved to the end.", "nothing else.", 1), wantErr: "treated as hand-written"},
		{shape: "resolver: the dependency-injection notice missing its last line", target: resolverOut,
			body: "package gen\n\n// This file will not be regenerated automatically.\n//\n", wantErr: "treated as hand-written"},
		{shape: "resolver: the follow-schema notice with an empty version", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(followNotice, " version v0.17.66", " version ", 1), wantErr: "treated as hand-written"},
		{shape: "resolver: a comment line where the blank line 2 belongs", target: resolverOut,
			body: "package gen\n// stray\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "resolver: the follow-schema notice with a FOREIGN third line", target: resolverOut,
			body:    "package gen\n\n" + strings.Replace(followNotice, "// Code generated by github.com/99designs/gqlgen version v0.17.66", "// Generated by hand", 1),
			wantErr: "treated as hand-written"},
		{shape: "resolver: the follow-schema notice with a version carrying a space", target: resolverOut,
			body:    "package gen\n\n" + strings.Replace(followNotice, "version v0.17.66", "version v0.17.66 and more", 1),
			wantErr: "treated as hand-written"},
		{shape: "resolver: the notice one line too low (two blank lines)", target: resolverOut,
			body: "package gen\n\n\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "resolver: the notice ABOVE the package clause", target: resolverOut,
			body: stubNotice + "\npackage gen\n", wantErr: "treated as hand-written"},
		{shape: "resolver: the notice MID-FILE", target: resolverOut,
			body: "package gen\n\ntype Resolver struct{}\n\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "resolver: a package clause that is not an identifier", target: resolverOut,
			body: "package 1gen\n\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "resolver: a package clause carrying a trailing comment", target: resolverOut,
			body: "package gen // x\n\n" + stubNotice, wantErr: "treated as hand-written"},
		{shape: "resolver: the notice with CRLF line endings", target: resolverOut,
		...
		{shape: "resolver: boundary (the notice block ends exactly at the scan window)", target: resolverOut,
			body: stubResolverOfLength(t, 4096)},
		{shape: "resolver: boundary+1 (the notice block ends one byte past the scan window)", target: resolverOut,
			body: stubResolverOfLength(t, 4097), wantErr: "treated as hand-written"},
	}
	...
	for i, tc := range cases {
		t.Run(cellID("G2", i)+" "+tc.shape, func(t *testing.T) {
			...
			if tc.plant != nil {
				...
				f.write(tc.target, tc.body)
			}
			if tc.wantErr != "" && strings.Contains(f.dir, tc.wantErr) {
				t.Fatalf("wantErr %q also occurs in the fixture path %s; the assertion could not tell a refusal from a path", tc.wantErr, f.dir)
			}
			before := f.digests()
			...
			if tc.plant == nil && !strings.Contains(report.String(), "provenance: "+tc.target+": ") {
				t.Fatalf("the accepted file's provenance decision was not reported; report:\n%s", report.String())
			}
		})
	}
}
