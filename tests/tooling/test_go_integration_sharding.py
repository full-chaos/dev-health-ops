...
def test_shard_plan_is_exhaustive_nonempty_and_machine_readable(
    ...
    # CURRENT TOTAL: 65 -- the one number to bump when a new
    ...
    # re-balanced.
    assert "integration shard plan: 6 shard(s), 65 package(s)" in result.stdout
    ...
    # CURRENT TOTAL: 65 -- the one number to bump.
    assert len(flattened) == len(set(flattened)) == 65
    ...
    # jira's native team-catalog collector (jira_team_catalog.go,
    # jira_team_catalog_route.go, jira_team_catalog_effects_clickhouse.go,
    # jira_team_catalog_guards.go) added 21 new top-level tests across
    ...
    # Deployment lifecycle timestamp derivation (a GitHub statuses fetch, a
    # GitLab nested deployable read, a rate-limit skip path, a
    ...
    # The same carry-forward over a failed lookup for a deployment's
    # merged_at/pull_request_number and a pull request's review-derived
    ...
    # Prepared-snapshot recovery for deployments and PR-social (the route
    # allow-list, the legacy-ledger and oversize fallbacks, the size-cap
    # boundary, the rollback cell, the per-route recovery-mode pin, the
    # provider-drift table, the superseded-snapshot discard and its decision
    # table, exact large integers on replay, and the discard reason labels)
    # added 13 ordinary tests and 2 integration-tagged tests: +15 top-level
    # (1507 -> 1522), integration-tagged 172 -> 174.
    # 1522 top-level provider tests, 174 of them integration-tagged; both
    # pins move with any top-level test added to or removed from
    # internal/providersync.
    assert len(expected_provider_tests) == 1522

    assert len(expected_integration_tests) == 174
    ...
    assert len(provider_flattened) == len(set(provider_flattened)) == 1522
...
def test_each_shard_dry_run_executes_only_its_manifest_assignment() -> None:
    ...
    # CURRENT TOTAL: 64 (== discovered-total-minus-one -- keep this in
    # sync with the discovered-total literal above when either changes).
    assert len(selected_packages) == len(set(selected_packages)) == 64
    ...
    assert len(selected_tests) == len(set(selected_tests)) == 1522
