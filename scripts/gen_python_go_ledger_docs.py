...
KIND_LEDGER: dict[str, dict[str, str]] = {
    ...
    "sync.team_autoimport": {
        ...
        "state": "mixed (migration-state below) — native for linear/github/gitlab sync-time dispatch (CHAOS-4431/4432/4434, Done), bridge for jira only (CHAOS-4198, not yet ported) among providers whose populate() actually writes rows -- pagerduty/launchdarkly sync runs also reach the bridge call mechanically but no-op there (not import-capable, see producer column); backfill dispatch is unchanged Python for all 4 populator-capable providers regardless of this kind's routing",
