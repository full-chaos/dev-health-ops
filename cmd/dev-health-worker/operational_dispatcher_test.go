...
// CHAOS-5384: a webhook-only worker no longer needs the HTTP bridge at all
// (CHAOS-5320/#2346 deleted the webhook leg), so it must construct no
// dispatcher and must not fail on an empty/unset bridge URL it never uses.
func TestBuildOperationalHTTPDispatcher_WebhookOnlyConstructsNone(t *testing.T) {
