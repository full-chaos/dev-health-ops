...
KIND_LEDGER: dict[str, dict[str, str]] = {
    ...
    "metrics.daily_partition": {
        ...
        "writer": "MIXED: Go-native for `team_wellbeing` (CHAOS-4276, `daily.go:212-234` `NewTeamWellbeingExecutor`), `repo_user_commit` (CHAOS-4275, `NewRepoUserCommitExecutor`), `incident` (CHAOS-4295, `NewIncidentExecutor` -- landed WITH the CHAOS-4269 fix: `active_incidents_query`'s `valid_from <= as_of` predicate has no NULL-OK guard, so Python's compute path for this family was permanently zero-yield for every `mapping_kind=\"repository_derived\"` incident mapping; the Go executor adds `valid_from IS NULL OR valid_from <= as_of`, mirroring the symmetric `valid_to` guard), `deploy` (CHAOS-4293, `NewDeployExecutor`, `internal/jobs/metrics/daily/deploy_native_executor.go`), and `work_item_state` (CHAOS-4278, `NewWorkItemStateExecutor` -- reads its team attribution from `work_item_team_attributions.is_primary=1` rather than recomputing the 9-source cascade, see that family's own row below; registered `phase=post_bridge` -- runs AFTER the bridge call below, not before, since it depends on `work_item_attribution`'s same-partition write); the other ~18 families still bridge via `internal/jobs/metrics/daily/compatibility_http.go` -> POST `/internal/worker/daily-metrics/v1/execute` -> `worker_metrics.py:1727` -> `src/dev_health_ops/metrics/job_daily.py:1104 run_daily_metrics_job` (NOT `workers/metrics_daily.py`, which is dead Celery-only code, see worker-file ledger)",
        "tables": "`public.daily_metrics_partitions` plus per-family ClickHouse output tables",
        "evidence": "argued — corrects both `.remember/chaos-3092-port-inventory-2026-08-25.md`'s '21/23 Python' framing (5 families are now native) and a prior fork's mis-citation of `workers/metrics_daily.py` (that file is dead; the live module is `src/dev_health_ops/metrics/job_daily.py`); `incident`'s zero-yield/fix claim is local — executed evidence in CHAOS-4269's ticket comment (2026-09-01: unfiltered `SELECT count() FROM incident_metrics_daily` = 0; predicate swap on a seeded org recovered 36/36 rows)",
        "state": "mixed (5 native, ~18 bridge)",
