...
func (resolver teamCatalogClientResolver) ResolveClient(
	...
	case "gitlab":
		...
		return credential, client, integrationID, err
	case "jira":
		client, err := providerfoundation.NewJiraClient(credential, resolver.doer, resolver.retry, lease)
		return credential, client, integrationID, err
	default:
...
// nativeTeamCatalogCollectors (internal/workerservice/sync_dispatch.go). A
