...
func TestRuntimeGrantStatementsMatchProvisionedLeastPrivilegePolicy(t *testing.T) {
	...
	want := []string{
		...
		"DO $$ BEGIN IF to_regclass('public.feature_flags') IS NOT NULL THEN GRANT SELECT, UPDATE ON TABLE public.feature_flags TO \"domain_runtime\"; END IF; END $$",
		"DO $$ BEGIN IF to_regclass('public.org_feature_overrides') IS NOT NULL THEN GRANT SELECT, UPDATE ON TABLE public.org_feature_overrides TO \"domain_runtime\"; END IF; END $$",
