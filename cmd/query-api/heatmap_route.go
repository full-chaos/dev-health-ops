...
func heatmapSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(heatmapEnabledEnvVar)); enabled {
...
// Postgres.
func loadHeatmapRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildHeatmapRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadHeatmapRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(heatmapSwitchFromEnv())
