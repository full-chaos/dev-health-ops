...
`sync.team_autoimport` is the kind that caused this ticket. Its dispatch is Go, gated by the OR of the org's 3 CHAOS-4323 flags (`auto_import_teams`/`projects`/`members` -- all 3 false means the job is never enqueued). **CHAOS-4492 update (2026-08-29):** as of #1989 (`27bef7286`, Linear), #1984 (`950752653`, GitHub), and #1985 (`5bff38a5a`, GitLab) -- all merged to main -- team/member/project-ownership writes for **linear, github, and gitlab are Go-native**, dispatched through a per-provider collector registry (`nativeTeamCatalogCollectors`, `cmd/dev-health-worker/sync_dispatch.go`). Only **jira** still writes through Python, reached through the narrow `reference-discovery-populate` bridge (synchronous, inside sync finalization) and the best-effort `team-autoimport` bridge (post-sync, fire-and-forget) -- both bridges now carry `exclude_providers` so linear/github/gitlab are skipped there. `sync.team_repo_ownership_derivation` is a *different*, fully Go-native producer downstream of those ownership tables -- it derives `team_repo_ownership` (source=`inferred`) from whichever ownership rows exist; it does not replace direct provider writes, and it is a lower-specificity signal.
...
flowchart LR
  ...
  K1 --> REG
  BDE --> REG
  REG -->|"linear #1989 27bef7286<br/>github #1984 950752653<br/>gitlab #1985 5bff38a5a"| NATIVE["Go-native collectors<br/>Collect(ctx, org, syncRun, selections)"]
  REG -->|"jira: no registered collector<br/>exclude_providers = [linear,github,gitlab]"| R1["POST /api/internal/worker-sync/team-autoimport"]
  REG -.->|"jira: same exclusion"| R2["POST /api/internal/worker-sync/reference-discovery-populate"]
  subgraph Python["Python (bridge target — jira only)"]
    ...
    TA --> POP["team_autoimport_jira.py populate()<br/>(linear/github/gitlab populate() now dead-in-practice,<br/>see worker-file ledger, pending CHAOS-4435 deletion)"]
    TAS --> POP
  end
...
Bridge routes `/team-autoimport` and `/reference-discovery-populate` still show as **live** above because jira still reaches them -- they are documented as "dead for linear/github/gitlab pending 5.6 prod readback" in the bridge-route table below, not fully dead, until prod proof exists (see [Known tracking gaps](#known-tracking-gaps-opened-from-this-page)).
...
| `sync.team_autoimport` | enqueue: `cmd/dev-health-worker/sync_dispatch.go:195 teamAutoimportPostSyncWriter.PublishTx`, called from `NativePostSyncService.publishTeamAutoimport` (`native_post_sync.go:337-344`) only when `plan.TeamAutoimport` is true; dequeue/consumer: `internal/syncdispatchruntime/worker.go:95 RegisterTeamAutoimportWorker`, wired `sync_dispatch.go:565`. Per-provider dispatch is now split at the collector registry (`cmd/dev-health-worker/sync_dispatch.go` `nativeTeamCatalogCollectors` map, added by #1989/#1984/#1985): linear/github/gitlab resolve to a registered Go collector; jira has none, so the bridge request carries `exclude_providers` for the ported three and jira still fans out to Python (corrected 2026-08-29, CHAOS-4492 — updates the 2026-08-28 'all Python' framing which predates the #1989/#1984/#1985 merges) | post-sync (best-effort, fire-and-forget) | Go-side: `plan.TeamAutoimport` (`native_post_sync.go:560-577`) = OR of the org's 3 CHAOS-4323 sync_options flags (`auto_import_teams`/`auto_import_projects`/`auto_import_members`) -- when all 3 are false, the job is never enqueued at all, not merely a no-op inside the populator. Provider routing gate: presence of a registered entry in `nativeTeamCatalogCollectors` (linear/github/gitlab present, jira absent) selects native collector vs. Python bridge fallback per provider. Python-side: `run_post_sync_team_autoimport` re-reads the same 3 sync_options flags independently for whatever providers it still receives; see CHAOS-4430 for the executed proof of the flag chain | linear/github/gitlab: Go-native collectors registered in `nativeTeamCatalogCollectors` (`cmd/dev-health-worker/sync_dispatch.go`), writing via `providerfoundation.CredentialResolver` + per-provider ClickHouse effects -- #1989 `27bef7286` (Linear), #1984 `950752653` (GitHub), #1985 `5bff38a5a` (GitLab), all merged to main. jira only: `bridge.go:113 TeamAutoImport` -> POST `/api/internal/worker-sync/team-autoimport` -> `worker_sync.py:269 team_autoimport_reference` -> `workers/team_autoimport.py:228 run_post_sync_team_autoimport` -> `team_autoimport_jira.py populate()` (not yet ported, CHAOS-4198) | ClickHouse `teams`, `team_memberships`, `team_project_ownership` | local — re-armed reference-discovery run, org 70d529e0, 2026-08-29 07:11 UTC (lane-4431-linear-route close-out + CHAOS-4431/4432/4434 close-out comments): outcome `native` for linear with `rows_written` for teams/team_memberships/team_project_ownership; ClickHouse `teams.updated_at` matches the re-arm instant; `system.query_log` writer `clickhouse-go/2.47.0`, not Python's `clickhouse-connect`. Superseded citation: the 2026-08-28 22:03 UTC readback of 102 Python-client `default.teams` inserts / 0 Go-client inserts predates these merges and no longer reflects the live path for linear/github/gitlab -- re-running that query against prod is scoped to the 5.6 readback (CHAOS-4492), not yet executed (no prod access from this worktree) | mixed (migration-state below) — native for linear/github/gitlab (CHAOS-4431/4432/4434, Done), bridge for jira only (CHAOS-4198, not yet ported) (migration-state: `go_default`/`river`) | CHAOS-4198 (jira populator, remaining scope) + CHAOS-4435 (delete kind/routes/modules once jira is native — blocked by CHAOS-4194→CHAOS-4193 dependency) + CHAOS-4492 (this ledger regen) |
...
| `/api/internal/worker-sync/reference-discovery-populate` | `bridge.go:133 PopulateReferenceDiscovery`, called from `bridge_discovery_executor.go:45`, wired into `NativeReferenceDiscoveryService`'s executor — LIVE, synchronous, blocking. As of #1989/#1984/#1985 the caller passes `exclude_providers` for linear/github/gitlab (routed to native Go collectors instead, see `sync.team_autoimport` kind row) | `worker_sync.py:286 reference_discovery_populate_reference` -> `reference_discovery.py:226` -> `team_autoimport.py:169 run_team_autoimport_strict` -> per-provider `populate()` — now reached for jira only | jira teams/members/memberships/project-ownership rows; credential decrypt + PagerDuty OAuth rotation Python-side. linear/github/gitlab excluded via `exclude_providers` | bridge — live for jira; dead for linear/github/gitlab pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven | CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native, blocked) + CHAOS-4492 (this ledger regen) |
| `/api/internal/worker-sync/team-autoimport` | `bridge.go:113 TeamAutoImport`, called from `RegisterTeamAutoimportWorker` (`worker.go:93`), wired `cmd/dev-health-worker/sync_dispatch.go:565` — LIVE, fire-and-forget. Same `exclude_providers` split as reference-discovery-populate for linear/github/gitlab | `worker_sync.py:269 team_autoimport_reference` -> `team_autoimport.py:228 run_post_sync_team_autoimport` -> same per-provider `populate()` — now reached for jira only | same tables as reference-discovery-populate, best-effort variant, jira only | bridge — live for jira; dead for linear/github/gitlab pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven | CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native, blocked) + CHAOS-4492 (this ledger regen) |
...
| `team_autoimport_github.py` | LIVE | imported by team_autoimport.py; ported CHAOS-4434 (`950752653`, Done) — Go-native collector now writes GitHub teams/team_memberships in prod-bound code, and the bridge request excludes github via `exclude_providers`, so `populate()` here is dead-in-practice (import-reachable, not execution-reachable) pending file deletion | CHAOS-4435 (delete once jira is native; blocked) |
| `team_autoimport_gitlab.py` | LIVE | imported by team_autoimport.py; ported CHAOS-4432 (`5bff38a5a`, Done) — Go-native collector now writes GitLab teams/team_memberships/team_project_ownership, and the bridge request excludes gitlab via `exclude_providers`, so `populate()` here is dead-in-practice pending file deletion | CHAOS-4435 (delete once jira is native; blocked) |
| `team_autoimport_jira.py` | LIVE | imported by team_autoimport.py; sole live writer of Jira team_project_ownership — the only provider still reaching `populate()` through the bridge; not yet split into its own porting child | CHAOS-4198 (not yet split into a Jira child) |
| `team_autoimport_linear.py` | LIVE | imported by team_autoimport.py; ported CHAOS-4431 (`27bef7286`, Done) — the previously-unwired Go route (CHAOS-3716) is now registered in `nativeTeamCatalogCollectors` and writes Linear teams/team_memberships/team_project_ownership, and the bridge request excludes linear via `exclude_providers`, so `populate()` here is dead-in-practice pending file deletion | CHAOS-4435 (delete once jira is native; blocked) |
