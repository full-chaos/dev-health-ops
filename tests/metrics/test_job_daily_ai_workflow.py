...
def test_skip_families_gates_compute_independently() -> None:
    """CHAOS-4280/CHAOS-4286 astra review finding 1: ``skip_families`` must
    gate ``_extract_ai_workflow_for_day``'s TWO computes independently --
    "ai_workflow" skips the PR-load + extract_ai_workflow_from_pull_requests
    call; "work_graph_edges" skips the review/deployment/incident-load +
    extract_review_deployment_incident_edges call. Naming one must never
    skip the other's queries or output, and this must gate the PYTHON
    COMPUTE itself (chris's 2026-09-05 CHAOS-3092 amendment), not just a
    write step further down in run_daily_metrics_job -- so this test spies
    on the queries actually issued, not merely on the returned lists.

    All 4 combinations of {ai_workflow, work_graph_edges} in skip_families:
    """

    def _fired(sink: _Sink, *substrings: str) -> bool:
        return any(any(s in q for s in substrings) for q in sink.queries)

    # (1) neither skipped: both computes run, exactly like the pre-existing
    ...
    assert _fired(sink, "FROM git_pull_requests", "FROM work_graph_issue_pr")
    assert _fired(
        sink,
    ...
    runs, artifacts, issues, reviews, deploys, incidents = result
    assert len(runs) == 1 and len(artifacts) == 1 and len(issues) == 1
    assert len(reviews) == 1 and len(deploys) == 1 and len(incidents) == 1

    # (2) "ai_workflow" only: PR-load + linkage-load never fire, runs/
    # artifact_edges/issue_edges empty; the OTHER family's queries and
    # output are entirely unaffected.
    sink = _Sink()
    result = _extract_ai_workflow_for_day(
        primary_sink=sink,
        org_id=ORG_ID,
        ...
        repo_id=None,
        repo_provider_by_id={str(REPO_ID): "github"},
        skip_families=frozenset({"ai_workflow"}),
    )
    assert not _fired(sink, "FROM git_pull_requests", "FROM work_graph_issue_pr")
    assert _fired(
        sink,
        "FROM git_pull_request_reviews",
        "FROM deployments",
    ...
    # (3) "work_graph_edges" only: review/deployment/incident-load never
    # fire, those three edge lists empty; ai_workflow's own queries and
    # output are entirely unaffected.
    sink = _Sink()
    result = _extract_ai_workflow_for_day(
        primary_sink=sink,
        ...
        repo_id=None,
        repo_provider_by_id={str(REPO_ID): "github"},
        skip_families=frozenset({"work_graph_edges"}),
    )
    assert _fired(sink, "FROM git_pull_requests", "FROM work_graph_issue_pr")
    assert not _fired(
        sink,
    ...
    runs, artifacts, issues, reviews, deploys, incidents = result
    assert len(runs) == 1 and len(artifacts) == 1 and len(issues) == 1
    assert reviews == [] and deploys == [] and incidents == []

    # (4) both skipped: NEITHER family's queries fire, everything empty --
    # naming both must not silently reproduce the single-gate (everything
    # runs) behavior, and must not silently run a compute the caller
    ...
    result = _extract_ai_workflow_for_day(
        ...
        repo_id=None,
        repo_provider_by_id={str(REPO_ID): "github"},
        skip_families=frozenset({"ai_workflow", "work_graph_edges"}),
    )
    assert sink.queries == []
    assert result == ([], [], [], [], [], [])
