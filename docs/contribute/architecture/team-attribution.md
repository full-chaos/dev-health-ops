...
> | GitLab Go-native route — `internal/providersync` (CHAOS-4432, PR #1985, stacked on #1989) | native via #1985 (walk parity with the bridge: a non-strict walk failure reports a `skipped` outcome instead of discarding already-fetched rows — same fix class as CHAOS-4431 round 2/3's cycles/projects partial-failure handling) | **Yes** | gitlab, when its native collector is reached (stacked on the shared base, **not yet merged to main**) |
>
> ```mermaid
> flowchart LR
