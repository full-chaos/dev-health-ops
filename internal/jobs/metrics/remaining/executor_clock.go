...
//
// This decision is per kind, not a house rule. RecommendationsExecutor takes
// the accessor instead, because its `now` is a caller-supplied parameter and
// the field is a genuine fallback rather than the sole source. Copying either
// choice across kinds mechanically is how the wrong one spreads.
