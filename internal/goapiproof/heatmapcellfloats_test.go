...
// This file pins heatmapCellFloats/heatmapDedupParity's own FloatTierB
// wiring (restcorpus.go) directly against the declared corpus Options,
// not a hand-built stand-in -- so a future edit that drops the
// FloatTierB field, or narrows/widens the declared path, fails here
// rather than only being noticed by a live prove run reporting false
// findings again.
...
func TestHeatmapDedupParity_TolerantOfULPNoiseAtHotspotMagnitude(t *testing.T) {
	...
	result := Compare(baseline, candidate, heatmapDedupParity)
...
// size.
func TestHeatmapDedupParity_TolerantOfULPNoiseAtReviewWaitMagnitude(t *testing.T) {
...
func TestHeatmapDedupParity_StillCatchesGenuineDifferenceAtHotspotMagnitude(t *testing.T) {
...
func TestHeatmapDedupParity_StillCatchesGenuineDifferenceAtReviewWaitMagnitude(t *testing.T) {
	baseline := snapshotFromJSON(t, heatmapCellsBody(heatmapCell("03", "Fri", 0.06666666666666667)))
...
// TestHeatmapDedupParity_RealHotspotRiskULPCaptureIsNowAMatch replays a
// real captured production deployed-vs-deployed prove run (GET
...
// reverse on the other 8. Before heatmapCellFloats existed, KeyedDirectionShape
// admitted the 7 direction-matched cells as if they were evidence of the
// declared repos-dedup mechanism (they are not -- that mechanism is
// strictly one-directional, as the review_wait_density capture below
// shows) and left the other 8 as a false uncovered finding. This is the
// falsifiable prediction: the next production prove of this route comes
// back a clean match, because data.axes carries only label arrays (see
// heatmapDedupParity's own doc comment) and no other leaf in this
// response is numeric.
func TestHeatmapDedupParity_RealHotspotRiskULPCaptureIsNowAMatch(t *testing.T) {
...
// TestHeatmapDedupParity_RealReviewWaitDensityCaptureUnchanged replays
// the SAME real captured production capture keyeddirection_test.go's own
// TestKeyedDirectionShape_RealHeatmapCapture already pins, but through
// the actual registered heatmapDedupParity (not the test-local
// heatmapCellsOptions stand-in), to prove wiring heatmapCellFloats does
// not touch this route's one live proof that the repos-dedup mechanism
// actually fires: both cells' real ~2x difference (0.0667 vs 0.0333) is
// seven orders of magnitude above the 1e-9 floor, so it still reaches
// classification and is still admitted by KeyedDirectionShape exactly as
// before Tier B existed.
func TestHeatmapDedupParity_RealReviewWaitDensityCaptureUnchanged(t *testing.T) {
