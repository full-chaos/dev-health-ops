...
func drilldownPRsSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(drilldownPRsEnabledEnvVar)); enabled {
...
// Postgres.
func loadDrilldownPRsRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildDrilldownPRsRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(drilldownPRsSwitchFromEnv(getenv))
	...
	entryHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		claims, ok := authenticateRESTRequest(w, r, verifier, edgeVerifier, "drilldown_prs")
		if !ok {
			return
		}
