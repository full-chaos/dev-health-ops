...
func TestTheWriteVerbEndToEndAgainstARealDatabase(t *testing.T) {
	...
	for _, ddl := range []string{registryschema.DDL, `CREATE TABLE wp_verb (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), org TEXT NOT NULL, run TEXT NOT NULL, name TEXT NOT NULL)`} {
	...
	buildinfo := httptest.NewServer(writeStaticJSONHandler(`{"commit":"` + build + `","modified":false}`))
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var run string
		if err := pool.QueryRow(r.Context(), `SELECT run FROM wp_verb WHERE name='seed' AND run NOT LIKE 'gwc-wp-probe-%' ORDER BY run DESC LIMIT 1`).Scan(&run); err != nil {
