...
func TestRunbookInvocationUsesRealFlags(t *testing.T) {
	documented := documentedInvocations(t, "go-api-prove")
	if len(documented) == 0 {
		t.Fatal("docs carry no go-api-prove invocation")
	}

	original, originalArgs := flag.CommandLine, os.Args
	t.Cleanup(func() { flag.CommandLine, os.Args = original, originalArgs })
	flag.CommandLine = flag.NewFlagSet("go-api-prove", flag.ContinueOnError)
	flag.CommandLine.SetOutput(io.Discard)
	os.Args = []string{"go-api-prove"}
	_, _ = parseFlags()
	...
	for _, invocation := range documented {
		...
		if err := flag.CommandLine.Parse(invocation.args); err != nil {
			t.Errorf("%s: the documented %s command does not parse: %v", invocation.doc, "go-api-prove", err)
		}
		for _, name := range invocation.flags {
			if flag.CommandLine.Lookup(name) == nil {
				t.Errorf("%s documents -%s, which go-api-prove does not define", invocation.doc, name)
			}
		}
	}
}
