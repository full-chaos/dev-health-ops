...
func opportunitiesSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(opportunitiesEnabledEnvVar)); enabled {
...
func buildOpportunitiesRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(opportunitiesSwitchFromEnv())
