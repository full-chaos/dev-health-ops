...
async def run_daily_metrics_finalize(
    *,
    ...
    # compute_ic_landscape_rolling) was deleted here (CHAOS-4290 PR3,
    # CHAOS-3092 no-straddle) -- the native Go executor has been the SOLE
    # writer for this family since #2241's finalize policy landed, so this
    # call was already dead weight, never a live fallback, before its
    # deletion (see PR3's body for the no-fail-open citation). `skip_families`
    # stays a parameter of this function: worker_metrics.py's finalize call
    # site still passes it through from the Go dispatcher's real skip list
    # for whichever finalize-scope family needs it next, even though nothing
    # in this function body currently checks it.
