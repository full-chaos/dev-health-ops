...
// explanation of this fixture and CHAOS-4818. This is the incidentPercentile
// copy of dev_health_ops.metrics.compute_incidents._percentile's
// `sorted[lo]*(1-frac) + sorted[hi]*frac` interpolation, tested against the
// SAME tests/fixtures/fma_golden.json rows as every other duplicate.
func TestIncidentPercentileMatchesLivePythonBitExact(t *testing.T) {
	...
	for _, testCase := range fixture.PercentileFloat {
		got := incidentPercentile(testCase.Values, testCase.Percentile)
		...
		if gotBits != wantBits {
			mismatches++
			if mismatches <= 10 {
				t.Errorf("incidentPercentile(%v, %v) = %v (bits %#x), want bits %#x (%v)",
