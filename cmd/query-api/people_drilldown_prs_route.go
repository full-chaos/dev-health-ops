...
func peopleDrilldownPRsSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(peopleDrilldownPRsEnabledEnvVar)); enabled {
...
func loadPeopleDrilldownPRsRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
// binary follows.
func buildPeopleDrilldownPRsRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadPeopleDrilldownPRsRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(peopleDrilldownPRsSwitchFromEnv())
	...
	entryHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		claims, ok := authenticateRESTRequest(w, r, verifier, "people_drilldown_prs")
		if !ok {
			return
		}
