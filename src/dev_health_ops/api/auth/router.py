...
from dev_health_ops.models.users import AuthProvider, Membership, Organization, User
...
class SocialLoginRequest(BaseModel):
    """Social login request with provider and access token from frontend OAuth flow."""
...
class SocialLoginResponse(LoginResponse):
    """Social login response — extends LoginResponse with is_new_user flag."""
...
async def social_login(
    ...
    """Verify a provider access token from the frontend OAuth flow and issue our own JWT pair.
    ...
    config = OAuthConfig(
        ...
        redirect_uri="https://localhost",  # Not used — we only call fetch_user_info
    ...
    try:
        ...
        logger.warning("Social login user info fetch failed for %s: %s", provider_name, exc)
        raise HTTPException(
            status_code=401,
            detail=error_detail("Invalid or expired provider token"),
        ) from exc
    except OAuthProviderError as exc:
        logger.error("Social login provider error for %s: %s", provider_name, exc)
        raise HTTPException(
            status_code=502,
            detail=error_detail("Failed to verify with provider"),
        ) from exc

    # Map provider string to AuthProvider enum value
    provider_enum = AuthProvider(provider_name)

    is_new_user = False

    async with get_postgres_session() as db:
        # 1. Try to find user by auth_provider + auth_provider_id
        result = await db.execute(
            select(User).where(
        ...
        if user is None:
            ...
            if user is not None:
                ...
                if existing_provider == AuthProvider.LOCAL.value:
                    # Allow: same person signing in via social
                    pass
