...
func TestRefundCreate(t *testing.T) {
	...
	refund("a JSON true amount is 1, as the request model coerces it", "super", `{"invoice_id":"`+invIntent+`","amount":true}`, 200,
		map[string]any{"stripe_payment_intent_id": "pi_listed", "amount": float64(1)})
	refund("bare charge from invoice payments", "super", `{"invoice_id":"`+invCharge+`"}`, 200,
		map[string]any{"stripe_charge_id": "ch_bare", "stripe_payment_intent_id": nil, "amount": float64(500)})
	...
	expect("refund rows written, by the superuser",
		...
		"in_A2=2000:succeeded:true,in_A2=4000:succeeded:true,in_charge=500:succeeded:true,in_pi=1:succeeded:true,in_pi=300:succeeded:true")
	expect("the listed intent is stored on the invoice",
		`SELECT string_agg(stripe_invoice_id || '=' || coalesce(payment_intent_id, '-'), ',' ORDER BY stripe_invoice_id) FROM invoices
			WHERE stripe_invoice_id IN ('in_pi', 'in_charge', 'in_none')`, "in_charge=-,in_none=-,in_pi=pi_listed")
