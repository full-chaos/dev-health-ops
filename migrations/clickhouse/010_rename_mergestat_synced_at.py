...
def upgrade(client):
    ...
    for table in all_tables:
        ...
        try:
            col_res = client.query(f"SELECT name FROM system.columns WHERE table = '{table}' AND name = '_mergestat_synced_at' AND database = currentDatabase()")
            if not col_res.result_rows:
                # Column doesn't exist, check if last_synced exists
                check_res = client.query(f"SELECT name FROM system.columns WHERE table = '{table}' AND name = 'last_synced' AND database = currentDatabase()")
        ...
        try:
            ...
            # Drop old table (now named table_new)
