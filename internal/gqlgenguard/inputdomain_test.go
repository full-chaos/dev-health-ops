...
// that a shape nobody thought about is a failing row here rather than a defect
// found later.
//
...
func TestGqlgenConfigParserInputDomain(t *testing.T) {
	cases := []domainCase{
		...
		// (Asserting ACCEPT here, or that a null decodes to the default, would
		// encode gqlgen's behaviour as the contract instead of the guard's.)
...
// the words. A substring scan accepts a hand-written file whose comment happens
// to quote the marker; the first rows below are that class, one row per place the words can sit without being gqlgen's header.
func TestProvenanceGuardInputDomain(t *testing.T) {
	...
	cases := []struct {
		...
		{shape: "exec: the words in a hand-written comment below the package clause", target: execOut,
