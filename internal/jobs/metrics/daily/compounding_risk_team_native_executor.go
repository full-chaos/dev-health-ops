...
// job_daily.py:613-675) reads back THIS RUN's already-computed
// repo_metrics_daily rows from ClickHouse rather than sharing in-process
// state with the daily_partition jobs that wrote them, because a Go
// finalize job shares no memory with them either -- this executor does the
// same readback (compoundingrisk.LoadRepoMetricsForOrgDay, org-wide, already
// argMax-deduped).
//
