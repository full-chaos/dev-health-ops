...
func sankeySwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(sankeyEnabledEnvVar)); enabled {
...
// Postgres.
func loadSankeyRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildSankeyRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadSankeyRouteConfig()
	if !cfgOK {
