...
func homeSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(homeEnabledEnvVar)); enabled {
...
func buildHomeRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(homeSwitchFromEnv())
