...
func workUnitsSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(workUnitsEnabledEnvVar)); enabled {
...
// registry Postgres.
func loadWorkUnitsRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildWorkUnitsRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadWorkUnitsRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(workUnitsSwitchFromEnv())
