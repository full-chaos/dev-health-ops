...
| --- | --- | --- |
| `DEV_HEALTH_GO_DHO_IMAGE` | `dev-health-go-dho` | the four `go-worker-*` processes (`dho worker`), `go-reconciler` (`dho reconciler`), `go-scheduler` (`dho scheduler`) and the three `go-stream-*` processes (`dho stream-runner`) |
