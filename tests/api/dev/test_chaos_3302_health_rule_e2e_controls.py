...
def _sustained(
    rule_id: str, *, periods: int, **kwargs: object
...
def test_positive_two_independent_at_risk_dimensions_qualify_team_needs_attention() -> (
    ...
    observations = {
        "health_rule.completion_stalled.v1": _sustained(
            "health_rule.completion_stalled.v1", periods=2
        ),
        "health_rule.review_latency_sustained.v1": _sustained(
            "health_rule.review_latency_sustained.v1", periods=2
        ),
    }
...
def test_positive_one_critical_rule_with_evidence_and_coverage_qualifies_team() -> None:
    observations = {
        "health_rule.data_trust_broken.v1": _sustained(
...
def test_negative_single_provisional_threshold_alone_produces_no_launch_finding() -> (
    ...
    observations = {rule_id: _sustained(rule_id, periods=3)}
...
def test_negative_cohort_below_minimum_suppresses_finding() -> None:
    ...
    observations = {
        "health_rule.completion_stalled.v1": _sustained(
            "health_rule.completion_stalled.v1", periods=2, cohort_size=small_cohort
        ),
    }
    result = evaluate_registry(HEALTH_RULE_REGISTRY, observations)
    assert result.launch_findings == ()
...
def test_negative_measured_zero_and_no_data_are_not_collapsed() -> None:
    ...
    measured_zero = _sustained(
        "health_rule.completion_stalled.v1",
        periods=2,
        data_semantics="measured_zero",
        current_value=0.0,
    )
    result_zero = evaluate_registry(
        HEALTH_RULE_REGISTRY, {"health_rule.completion_stalled.v1": measured_zero}
    )
    zero_finding = (result_zero.launch_findings + result_zero.suppressed_findings)[0]
    ...
    result_no_data = evaluate_registry(
        HEALTH_RULE_REGISTRY, {"health_rule.completion_stalled.v1": no_data}
    )
    no_data_finding = (
        result_no_data.launch_findings + result_no_data.suppressed_findings
    )[0]
...
def test_negative_provisional_rule_construction_requires_shadow_and_no_evidence_claim() -> (
    ...
    from dev_health_ops.api.dev.contracts_v2.health_rules import HealthRuleDefinition
