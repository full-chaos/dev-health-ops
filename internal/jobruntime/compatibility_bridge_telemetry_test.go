...
func TestCompatibilityBridgePartitionCountersReachTheExposition(t *testing.T) {
	...
	// not be conflated with an observed zero. membership_backfill (the
	// original example here) is CHAOS-4282's native cutover and left the
	// bridge's closed family set; complexity is still bridge-routed and
	// serves the same purpose.
	if err := collector.ObserveCompatibilityPartition("complexity", nil); err != nil {
		t.Fatalf("observe: %v", err)
	}
	...
	for _, want := range []string{
		...
		`worker_remaining_bridge_rows_written_total{family="complexity"} 0`,
		`worker_remaining_bridge_zero_row_partitions_total{family="complexity"} 0`,
	} {
