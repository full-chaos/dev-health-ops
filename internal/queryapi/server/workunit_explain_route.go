...
func workUnitExplainSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(workUnitExplainEnabledEnvVar)); enabled {
...
func buildWorkUnitExplainRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(workUnitExplainSwitchFromEnv(getenv))
