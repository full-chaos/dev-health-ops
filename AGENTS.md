...
- **Mutation testing uses the shared harness — [`scripts/mutation_harness.py`](scripts/mutation_harness.py). Do not write your own.** Three ad-hoc per-lane harnesses produced false results on 2026-07-26, all the same shape: the harness could not detect its own failure. One left `if false && (guard)` on disk while reporting a restore it had "verified" with `go build`; one used `git checkout` to restore and silently reverted unrelated uncommitted edits; one waited on `pgrep -qf "m22.sh"`, matched its own command line, and hung. The harness snapshots and asserts a **content digest** on restore, records the applied mutation on disk *before* touching the file so a killed run stays detectable, refuses anchors that match a different count than declared or land on comment lines, and requires each proof to go green → red → green. `verify` is the first stage of `ci/local_validate.sh`. Two rules it cannot enforce for you: **never verify a restore with a build or a git check** (`go build`/`go vet` pass on `if false && …`; `git diff` calls an *untracked* file clean whatever it contains), and **mutate compound predicates clause by clause** — a wholesale mutation reported KILLED on a condition holding a wrong, unasserted clause. If `restore` refuses — snapshot deleted, or the file reconciled by hand and matching neither digest — fix the file and clear the record with `accept --digest <sha256 of the file>`, which checks the mutation's text is actually gone rather than taking your word for it; never delete `.mutation-harness/state.json` by hand. **Scope (CHAOS-3165):** the obligation to author a mutation plan for new work is scoped to the Go port and is removed when CHAOS-3033 finalises — CHAOS-3165 tracks that removal and is blocked by CHAOS-3033. That is scope, not doubt: the requirement is load-bearing until the port closes, and the `verify` gate stage outlives it either way. Runbook: [`tests/tooling/README.md`](tests/tooling/README.md).
