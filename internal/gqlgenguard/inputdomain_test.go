...
func TestProvenanceGuardInputDomain(t *testing.T) {
	...
	cases := []struct {
		...
		{shape: "exec: the header with NO terminating newline (the whole file)", target: execOut,
			body: headerLine, wantErr: "hand-written"},
		...
		{shape: "exec: a RESOLVER notice on an exec output (a shape gqlgen never writes there)", target: execOut,
			body: "package gen\n\n" + stubNotice, wantErr: "hand-written"},
		{shape: "exec: zero (an empty file)", target: execOut, body: "", wantErr: "hand-written"},
		...
		{shape: "resolver: the dependency-injection notice with its LAST line reworded", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(stubNotice, "add any dependencies you require here.", "written by a person.", 1), wantErr: "hand-written"},
		{shape: "resolver: the follow-schema notice with its SECOND line reworded", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(followNotice, "any unknown code will be moved to the end.", "nothing else.", 1), wantErr: "hand-written"},
		...
		{shape: "resolver: the follow-schema notice with an empty version", target: resolverOut,
			body: "package gen\n\n" + strings.Replace(followNotice, " version v0.17.66", " version ", 1), wantErr: "hand-written"},
