...
// variable, in the same shape goapicli/routing and goapicli/prove use for
// this seam, so a test can capture parseFlags' own usage output without
// reaching into a process-global flag.CommandLine.
var flagOutput io.Writer = os.Stderr //nolint:gochecknoglobals // see prove's own flagOutput for the same seam

func registerFlags() (*flag.FlagSet, *flags) {
	f := &flags{}
	f.binds = map[string]string{}
	fs := flag.NewFlagSet("go-api-rest-prove", flag.ContinueOnError)
	fs.SetOutput(flagOutput)
	fs.Func("bind", "an operator-supplied id binding for a corpus request whose IDBindings names a goapiproof.IsOperatorSuppliedIDProducer producer, as NAME=VALUE. Repeatable, once per name. NAME must match a producer the corpus actually declares operator-suppliable (an unmatched NAME is simply never consumed); VALUE is never validated beyond being non-empty, since its shape is the corpus request's own concern. Rejected immediately, before any request is planned or sent, when the argument carries no \"=\" or either side of it is empty", func(raw string) error {
	...
	fs.StringVar(&f.org, "org", "", "org id every request is made for (required)")
	...
	fs.StringVar(&f.keyID, "key-id", "", "envelope signing key id (kid) -- a public identifier")
	fs.StringVar(&f.artifactDir, "artifact-dir", "", "directory for content-addressed leg bodies and finding lists; go_api_rest_proof_run's baseline/candidate_response_ref name the two legs, and review_evidence names the finding-list ref -- never an inlined body (required), matching go-api-prove's own -artifact-dir")
