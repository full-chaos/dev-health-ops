...
func TestCheckPostureAcceptsExactMatch(t *testing.T) {
	...
	conn := h.newUser(t, grantTeamsExact, grantIdentitiesExact)
...
func TestCheckPostureRejectsMissingPrivilege(t *testing.T) {
	h := startPostureHarness(t)
	conn := h.newUser(t, "GRANT SELECT, INSERT ON default.teams", grantIdentitiesExact)
...
func TestCheckPostureRejectsExtraGrant(t *testing.T) {
	h := startPostureHarness(t)
	conn := h.newUser(t, grantTeamsExact, grantIdentitiesExact, "GRANT SELECT ON default.other_table")
	err := CheckAPIClickHouseAuthorization(h.ctx, conn)
	if err == nil {
...
func TestCheckPostureRejectsExtraPrivilegeOnDeclaredTable(t *testing.T) {
	h := startPostureHarness(t)
	conn := h.newUser(t,
		"GRANT SELECT, INSERT, ALTER DELETE, ALTER UPDATE ON default.teams", grantIdentitiesExact)
	err := CheckAPIClickHouseAuthorization(h.ctx, conn)
	if err == nil {
