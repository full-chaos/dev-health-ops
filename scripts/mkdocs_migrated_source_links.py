...
def _rewrite_url(url: str, source_path: str, *, image: bool) -> str:
    ...
    rewritten = urlunsplit(("https", urlsplit(origin).netloc, f"/{'/'.join(origin.split('/')[3:])}/{resolved}", parsed.query, parsed.fragment))
...
def _rewrite_markdown(markdown: str, source_path: str) -> str:
    ...
    for line in markdown.splitlines(keepends=True):
        ...
        if reference:
            ...
            output.append(
                f"{reference.group('prefix')}{rewritten}{title}{reference.group('suffix')}"
            )
            continue
