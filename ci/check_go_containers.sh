...
# smoke_migrate_clickhouse runs `dho migrate clickhouse status` with no
# database configured: the verb must be reached, load both embedded head
# baselines, and fail closed (exit 1) with the JSON configuration error naming
# CLICKHOUSE_URI -- not an unknown command (exit 2) or a missing baseline.
smoke_migrate_clickhouse() {
  ...
  code=$?
  set -e
  [ "${code}" = "1" ] && printf '%s' "${output}" | grep -F 'CLICKHOUSE_URI is required' >/dev/null \
    || die "${tag}: dho migrate clickhouse status under contract 2 did not reach the DSN check: ${output}"
}
