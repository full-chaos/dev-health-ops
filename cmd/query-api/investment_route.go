...
func investmentSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(investmentEnabledEnvVar)); enabled {
...
func investmentSunburstSwitchFromEnv() *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(os.Getenv(investmentSunburstEnabledEnvVar)); enabled {
...
// registry Postgres.
func loadInvestmentRouteConfig() (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
// binary follows.
func buildInvestmentRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadInvestmentRouteConfig()
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(investmentSwitchFromEnv())
...
func buildInvestmentSunburstRoute() (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadInvestmentRouteConfig()
	if !cfgOK {
		return nil, nil, false, nil
	}
	...
	routeMux := routeswitch.NewMux(investmentSunburstSwitchFromEnv())
