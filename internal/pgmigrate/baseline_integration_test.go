...
// updateEnv, set to 1, rewrites baseline/<variant>.json from the executed
...
// differential oracle, for each cutover variant:
//
//  1. capture: run the REAL Python upgrade on a fresh database and dump it
//     (pg_dump, from the server's own image). The checked-in baseline must
//     equal that capture (drift check, by execution).
//  2. oracle: `pgmigrate.Upgrade` on a second fresh database must produce a
//     byte-identical schema dump and data dump.
//  3. a second Upgrade applies nothing; a database missing a head is refused
//     as below the head; an application head read with the cutover
//     authorized is below the head by 0066; a public schema without
...
func TestBaselineIsTheExecutedPythonUpgrade(t *testing.T) {
	...
	for _, cutover := range []bool{false, true} {
		t.Run(pgmigrate.Variant(cutover), func(t *testing.T) {
			if cutover {
				t.Setenv(pgmigrate.CutoverEnv, "1")
			} else {
				t.Setenv(pgmigrate.CutoverEnv, "")
			}
			...
			captured := capture(t, ctx, instance, pythonDB, cutover, window{pythonStarted, time.Now()})

			checkedIn := captured
			if os.Getenv(updateEnv) == "1" {
				writeBaseline(t, captured)
			} else if checkedIn, err = pgmigrate.LoadBaseline(cutover); err != nil {
				t.Fatal(err)
			}
			if diff := compare(captured, checkedIn); diff != "" {
				t.Fatalf("baseline/%s.json is not what the Python upgrade builds today (%s); regenerate it with "+
					"%s=1 go test -tags=integration -run TestBaselineIsTheExecutedPythonUpgrade ./internal/pgmigrate",
					pgmigrate.Variant(cutover), diff, updateEnv)
			}

			// A baseline that fails part-way leaves nothing: schema and data
			...
			// The oracle compares CANONICAL dumps. pg_dump output does not
			// survive a restore byte for byte: Postgres re-parses some
			// expressions it deparsed (a varchar IN list becomes per-element
			// casts, nested ANDs flatten), so the dump of any restored
			// database differs from its source in those lines while meaning
			// the same. Passing both databases through the same one
			// dump-and-restore puts them in one form, and there they must be
			// byte-identical.
			pythonCanonical := roundTrip(t, ctx, instance, admin, captured)
			goCanonical := roundTrip(t, ctx, instance, admin, capture(t, ctx, instance, goDB, cutover, goWindow))
			...
			if !cutover {
				cutoverBaseline, err := pgmigrate.LoadBaseline(true)
				if err == nil {
					_, err = pgmigrate.Upgrade(ctx, goConn, cutoverBaseline, chain)
					var below pgmigrate.BelowHeadError
					if !errors.As(err, &below) || !reflect.DeepEqual(below.Missing, []string{"0066"}) {
						t.Fatalf("an application head upgraded with the cutover authorized = %v, want below the head by 0066", err)
					}
				} else if os.Getenv(updateEnv) != "1" {
					t.Fatal(err)
				}
			}

			head := checkedIn.Heads[len(checkedIn.Heads)-1]
			if _, err := goConn.Exec(ctx, "DELETE FROM alembic_version WHERE version_num = $1", head); err != nil {
				t.Fatal(err)
			}
			_, err = pgmigrate.Upgrade(ctx, goConn, checkedIn, chain)
			var below pgmigrate.BelowHeadError
			if !errors.As(err, &below) || !reflect.DeepEqual(below.Missing, []string{head}) {
				t.Fatalf("upgrade with %s unrecorded = %v, want below the head naming it", head, err)
			}
...
func roundTrip(t *testing.T, ctx context.Context, instance *containers.Instance, admin *pgx.Conn, dump pgmigrate.Baseline) pgmigrate.Baseline {
	...
	return capture(t, ctx, instance, database, dump.Cutover, window{started, time.Now()})
}
...
func capture(t *testing.T, ctx context.Context, instance *containers.Instance, database string, cutover bool, migrated window) pgmigrate.Baseline {
	t.Helper()
	...
	return pgmigrate.Baseline{Cutover: cutover, Heads: heads, Schema: schema, Data: data}
}
...
func writeBaseline(t *testing.T, baseline pgmigrate.Baseline) {
	...
	path := filepath.Join("baseline", pgmigrate.Variant(baseline.Cutover)+".json")
