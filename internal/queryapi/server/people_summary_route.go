...
func peopleSummarySwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(peopleSummaryEnabledEnvVar)); enabled {
...
func loadPeopleSummaryRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
func buildPeopleSummaryRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	...
	routeMux := routeswitch.NewMux(peopleSummarySwitchFromEnv(getenv))
	...
	entryHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		claims, ok := authenticateRESTRequest(w, r, verifier, edgeVerifier, "people_summary")
		if !ok {
			return
		}
