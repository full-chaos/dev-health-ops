...
async def test_skip_families_naming_unrelated_family_has_no_effect(
    ...
    assert "repo_metrics" in sink.write_calls
    # CHAOS-5234/CHAOS-3092: cicd's compute+write is deleted outright now
    # (see test_cicd_compute_and_write_are_deleted_from_job_daily below), so
    # it can no longer serve as the "real, non-empty row" unrelated-family
    # control the way the codex round-4 (CHAOS-4292) finding originally
    # wanted -- write_deploy_metrics takes its place. It is a weaker check
    # (deploy fires unconditionally regardless of row content in this
    # fixture, which has no deploy data source), but "unrelated write still
    # happens at all" is the property this test is actually pinning.
...
async def test_deploy_skip_does_not_affect_other_families(
    ...
    """Naming deploy in skip_families must not perturb repo_metrics or any
    other family's write path."""
    sink = _RecordingSink("clickhouse://test")
    _neutralize_daily_job(monkeypatch, sink=sink, loader=_FakeLoader())
...
async def test_ai_governance_compute_and_write_are_deleted_from_job_daily(
    ...
    for skip_families in (None, {"ai_governance"}):
        ...
        # deploy (unrelated family, same partition) must be entirely
        # unaffected by the deletion. (Was cicd -- CHAOS-5234/CHAOS-3092
        # deleted cicd's own compute+write in this same PR, so it can no
        # longer serve as this control.)
        assert "write_deploy_metrics" in sink.write_calls
