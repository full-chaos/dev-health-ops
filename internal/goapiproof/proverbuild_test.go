...
func TestProverBuildLine(t *testing.T) {
	for _, tc := range []struct {
		...
		{"same", NewProverBuild(version.Info{Commit: "abc"}, "abc", false), "prover_build=abc prover_build_modified=false candidate_build=abc prover_build_skew=false"},
		{"skew refused", NewProverBuild(version.Info{Commit: "abc"}, "def", false), "prover_build=abc prover_build_modified=false candidate_build=def prover_build_skew=true"},
