...
2. **Materialization**: Scans for runs that need dispatching, finalization, or post-sync processing, then creates outbox rows for them. Where an outbox row already exists, an `ON CONFLICT` guard decides whether to reset it to `pending`, and that guard is deliberately **not** the same for every kind, because the coordinator role does not have the same evidence about every kind.
   - `dispatch_sync_run` re-arms a `dispatched`+`river` row when the run still has work the delivery never picked up: a unit `dispatching` past `SYNC_UNIT_DISPATCH_STALE_SECONDS`, a unit `retrying` past its `available_at`, or (**CHAOS-4359**) a unit still `planned` behind a delivery whose own `dispatched_at` is past that same stale window. The `planned` disjunct is the one that needs the delivery-side clock: a unit is `planned` for the whole interval between publishing `dispatch_sync_run` and that job claiming its units, so admitting it without the grace window would reset a freshly published row on the next tick and make the kernel publish a duplicate dispatch job every second until the first one ran (the CHAOS-4357 round-2 amplification, on a different kind). Before CHAOS-4359 the `planned` case was selected as a candidate and then silently refused, and nothing else re-arms a `dispatch_sync_run` delivery — `TerminalDeliveryRepair`'s three River-terminal branches all require a `discarded`/`cancelled` job, which a delivery that *completed* does not have. Measured on the local stack 2026-09-09: `sync_run 837d069b` held eight `planned` units behind a `dispatched`+`river` row frozen at `attempts = 9` for eleven days.
   - `finalize_sync_run` refuses **every** `dispatched`+`river` row, and that refusal is deliberate. `finalizeReadyRunPredicate` proves only that the run is *ready* to finalize, which stays true from the moment the finalize job is published until it completes — so there is no domain-side clock that separates a dead delivery from a working one, and the coordinator role can read neither `river_job` nor `sync_dispatch_transport_routes` to get one. Re-arming here would both double-deliver a live finalize and resurrect stale route generations. Step 4 is the licensed owner of that class.
...
4. **Ready-finalizer backstop (CHAOS-5456)**: Re-arms a `finalize_sync_run` row still `dispatched` whose River delivery ended **completed** (or whose River row was reaped) while the run never reached a terminal status. Materialization at step 2 selects such a run every pass and writes nothing — its `ON CONFLICT` guard cannot replace a dispatched River row from the coordinator side, where a live delivery and a dead one look identical — so without this step the run is stranded permanently and silently. The backstop runs on the queue-control pool, where River's state is readable, and its write is licensed by the *same* readiness predicate materialization uses, read on the coordinator pool through a SEPARATE connection, taken while the queue transaction holds the outbox row locked (`FOR UPDATE SKIP LOCKED`). The two are deliberately not one transaction — the queue role has no grant on the coordinator-only ledgers the readiness verdict rests on, so the foreign read moves out of the queue transaction rather than either role being widened. A `dispatched` generation that no longer matches the current route generation is excluded and stays with the audited route-history recovery (CHAOS-5462). Every pass logs `syncreconciler.ready_finalize_pass` with all counters present, zeros included.
5. **Orphaned-unit repair (CHAOS-5453)**: Mints a **new** `worker_job_outbox` row for a `sync.provider_unit` unit stuck `dispatching` whose existing row is terminal (`delivered`) and whose River job **completed** or was reaped. Step 3's relay never claims a `delivered` row again, and the dispatcher's own republish finds that row by dedupe key and returns `ErrDeliveryAlreadyTerminal` — so every redispatch pass is a no-op reported as progress (run `1410329c` reached 1,344 of them over eight days). The terminal row is never patched in place: `worker_job_outbox.dedupe_key` is UNIQUE, and re-arming the key would also let River's `ByArgs` unique index — whose default state set **includes** `completed` — dedupe the replacement into the finished job. The replacement therefore carries a reclaim key, `sync.provider_unit:<unit id>/reclaim/<n>`, which is both the provenance and the bound: three generations, then the repair stops and counts `skipped_reclaim_budget`. `discarded`/`cancelled` deliveries are refused here and left to the strand repair and the unreclaimable sweep. The survey reads `river_job` on the queue-control pool and the INSERT runs on the domain pool, because those are the only roles granted each half. The idle gate is `COALESCE(last_heartbeat_at, created_at)`, never `updated_at`, which the redispatch loop itself keeps fresh. A delivery is only acted on when it is provably this row's own (positive `river_job_id`, matching job kind, and relay metadata naming this outbox row) and the row's envelope agrees with itself (`payload.unit_id` == `domain.id`, which `jobcontract` does not enforce but the handler does); each refusal has its own counter. Every pass logs `syncreconciler.orphaned_unit_pass` with all counters present, zeros included.
