...
# CHAOS-6244's whole reason to exist as a SEPARATE pair from
# jira/incidents/entitlement (same feature_policy.decide_feature function,
# same test harness): agent_context_runtime is a member of
# EXPLICIT_PURCHASE_FEATURES (licensing/registry.py) and
# canonical_incident_ingestion is not, so the two keys take a DIFFERENT
# branch at decide_feature's tail (explicit-purchase-required vs
# enabled-by-tier) for the exact same {registered, globally_enabled, no
# override} input. A pair that reused "canonical_incident_ingestion" here
# would not exercise that branch at all.
...
def _reflected_fields() -> frozenset[str]:
    if pathlib.Path(feature_policy.__file__).resolve(strict=True) != _SOURCE.resolve(
        ...
        raise RuntimeError("acr entitlement oracle did not import the live policy")
...
oracle_registry.register(
    oracle_registry.PairSpec(
        id="acr/entitlements/agent-context-runtime",
