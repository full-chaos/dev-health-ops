...
def _run_latest_tag_step(
    ...
    eleven identically and none of them reach the :latest check at all
...
def test_latest_check_behaviourally_bootstraps_only_on_confirmed_absence() -> None:
    ...
    # source_tag_scenario override), so all eleven hit `:latest` UNKNOWN --
    ...
    assert unknown_rc != 0, (
        "all eleven families hitting :latest UNKNOWN should now fail the "
    ...
    # all eleven families hit the empty-read case, which is now ALSO an
    ...
    assert empty_rc != 0, (
        "all eleven families hitting the rc==0-yet-empty :latest read should "
...
def test_source_tag_check_distinguishes_confirmed_absent_from_unknown() -> None:
    ...
    `imagetools create` for any of the eleven families, which all share one
    ...
    assert unknown_rc != 0, (
        "all eleven families hitting source_rc==2 should now fail the step "
...
def test_source_tag_all_unknown_fails_the_job_instead_of_a_silent_noop() -> None:
    ...
    per-family gap -- it hits all eleven simultaneously. Before this row's
    fix, that produced eleven `::error::` annotations, an EMPTY create
    ...
    own "absent" call (all eleven hit source_rc==1, not 2) uses the
    ...
    assert "0 of 11 families had a moving tag applied this run" in stdout, (
...
def test_latest_check_all_unknown_fails_the_job() -> None:
    ...
    only counted source-tag-stage ambiguity. An all-eleven-source-PRESENT
    ...
    "always found"), so all eleven reach `:latest`; `scenario="unknown"`
    then answers UNKNOWN for all eleven uniformly. Verified this row goes
...
def test_latest_check_all_families_digest_walk_exhausted_fails_the_job() -> None:
    ...
    digest walk finds nothing for any of the eleven. Verified this row
    ...
    assert returncode == 1, (
        f"expected the all-eleven digest-walk-exhausted scenario to exit "
...
def test_latest_check_all_families_disagree_fails_the_job() -> None:
    ...
    Every family shares the same $SCENARIO ("disagree"), so all eleven
    ...
    assert returncode == 1, (
        f"expected the all-eleven platform-label-disagree scenario to "
