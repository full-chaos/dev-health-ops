...
// exit 3 instead of a spurious alert; the previous default cost the
// reverse, repeatedly. `errInternal` is the explicit opt-out.
//
// Exit codes follow the dho binary's shared contract (internal/cli): this
// command has always had exactly two buckets -- errInternal (a defect in
// the command itself) and everything else (a refusal: an unknown verb, a
// bad flag, a missing precondition, are all environmental, by the design
// documented above). Before this fold BOTH used exit 2. The fold spec
// (S1, §3.4) moves the refusal bucket to exit 3, matching cli.ExitRefused
// ("a preflight said no and nothing was written") -- every consumer that
// tests this exit code (deploy scripts/verify-query-api-plane.sh,
// ci/lib/go_api_prove_e2e.sh) is updated in the same PR.
