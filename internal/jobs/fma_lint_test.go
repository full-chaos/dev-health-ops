...
// addition or subtraction (spelled explicitly, e.g. `a*b + c`, or implicit
// in a compound assignment, e.g. `total += a*b`). This is exactly the shape
// CHAOS-4818's ReleaseImpactConfidence and its nine sibling fixes closed:
// the Go spec permits fusing such an expression into a single
// fused-multiply-add on arm64 (one rounding) where CPython always rounds
// the multiply and the add/subtract separately.
...
// `0.35*a + 0.35*b`, AND the compound-assignment spelling of the same
// shape, `total += a*b` / `total -= a*b` (the addition is implicit in the
// `+=`/`-=` token rather than a literal `*ast.BinaryExpr`, so it needs its
// own detection path -- see directOperandOfUnguardedAddSub). It does NOT
// flag the fixed idiom `float64(0.35*a) + float64(0.35*b)` or
// `total += float64(a*b)`, because the multiplication's immediate
// (paren-stripped) parent is then a conversion call to float64/float32,
// which the Go spec documents as forcing rounding and preventing fusion.
//
// It also flags one level of indirection through a non-conversion wrapper
// call -- `identity(a*b) + c` and `total += identity(a*b)` -- because codex
// round 1 (on the original expression-only version of this check)
// constructed exactly that (EXECUTED: arm64 assembly showed `FMADDD` for
// it) and a check that looked only at the MUL's immediate parent missed it
// (the immediate parent there is the CallExpr, not the +/-).
//
...
// it structurally cannot catch, both real, both found by this sweep (the
// second is what closed the FIRST version of this lint's compound-
// assignment blind spot: codex round 3 on PR #2106 found three live sites
// in computePipelineStability, `successRate7d +=`/`num +=`/`den +=`, that
// the expression-only checker never inspected because none of them contain
// a literal `+`/`-` *ast.BinaryExpr* -- the addition is the `+=` token
// itself):
//
...
func TestNoUnguardedFloatFMAInJobsPackages(t *testing.T) {
	...
	if len(findings) > 0 {
		...
		b.WriteString("unguarded float x*y +/- z site(s) found -- Go may fuse these into one FMA on arm64, rounding differently than CPython (CHAOS-4818). Wrap each product: float64(a*b) + float64(c*d), or total += float64(a*b) for a compound assignment.\n")
...
// expression case (must be flagged), one float64()-wrapped expression case
// (must NOT be flagged), one integer expression case that looks the same
// syntactically (must NOT be flagged -- integer multiply-add is exact,
// never FMA-exposed), codex round 1's construction on the original
// expression-only checker (a non-conversion wrapper call around the
// multiply, must be flagged), codex round 2's two constructions (a
// parenthesized sibling operand, and a locally shadowed `float64`
// identifier, both must be flagged), and codex round 3's construction on
// PR #2106 (compound assignment, `total += a*b`, must be flagged, plus a
// guarded and an integer compound-assignment case that must NOT be).
...
func CompoundAssignAccumulator(a, b float64) float64 {
	var total float64
	total += a * b
	return total
}

func CompoundAssignSubtract(a, b float64) float64 {
	var total float64
	total -= a * b
	return total
}

func CompoundAssignWrappedByNonConversionCall(a, b float64) float64 {
	var total float64
	total += identity(a * b)
	return total
}

func GuardedCompoundAssign(a, b float64) float64 {
	var total float64
	total += float64(a * b)
	return total
}

func IntegerCompoundAssign(a, b int) int {
	var total int
	total += a * b
	return total
}
`
	...
	// float64() wrap, matching every real fix in this repo.
	// WrappedByNonConversionCall's a*b is the codex round-1 P1 repro.
	// ParenSibling and ShadowedFloat64Conversion are codex round-2's two P1
	// repros. CompoundAssignAccumulator, CompoundAssignSubtract and
	// CompoundAssignWrappedByNonConversionCall are codex round-3's P1 repro
	// class (compound assignment) plus its own non-conversion-wrapper
	// variant. GuardedCompoundAssign and IntegerCompoundAssign are the
	// negative controls for that same class -- neither must be flagged.
	findings := parseAndCheckFixture(t, src)
	const want = 8 // Unguarded's a*b, Unguarded's c*d, WrappedByNonConversionCall's a*b,
	// ParenSibling's a*b, ShadowedFloat64Conversion's a*b,
	// CompoundAssignAccumulator's a*b, CompoundAssignSubtract's a*b,
	// CompoundAssignWrappedByNonConversionCall's a*b.
	if len(findings) != want {
		t.Fatalf("got %d findings, want exactly %d: %+v", len(findings), want, findings)
	}
}
...
// that is itself the direct (paren-stripped) operand of a +/- BinaryExpr or
// of a compound +=/-= assignment, with no intervening float64()/float32()
// conversion call.
//
...
// or a compound +=/-= assignment, either DIRECTLY, or through exactly one
// wrapping call that is NOT a float64()/float32() conversion.
//
// The compound-assignment branch exists because of a codex round-3 finding
// on PR #2106 (P1, EXECUTED): `total += a*b` has no literal `+`
// *ast.BinaryExpr* anywhere in its AST -- the addition is entirely implicit
// in the `token.ADD_ASSIGN` token of the enclosing *ast.AssignStmt*, so a
// checker that only recognises `isAddSubBinary` parents (the shape below,
// before this fix) never visits it at all. `total`'s three real sites in
// computePipelineStability (successRate7d, num, den) were found only by
// `go tool objdump` showing `FMADDD`/`FMSUBD` in the compiled arm64 binary,
// after the lint itself reported clean. mul's parent being the AssignStmt
// directly is exactly the same shape as mul being a direct BinaryExpr
// operand: in both cases mul is a bare expression sitting in a field of its
// parent node (X/Y for BinaryExpr, one element of Rhs for AssignStmt), so
// the same "is my immediate non-paren parent an implicit-or-explicit
// addition" test applies to both, and the same one-level non-conversion
// call indirection (`total += identity(a*b)`) needs the same allowance the
// expression case already had.
//
// The non-conversion-wrapper case (both branches) exists because of a
// codex round-1 finding (P1, EXECUTED): `identity(a*b) + c`, where
// `identity` is a small Go function that returns its argument unchanged,
// is NOT protected the way `float64(a*b) + c` is -- only an explicit
// conversion is documented by the Go spec as forcing rounding, so a
// generic wrapper call gives no such guarantee, and if it gets inlined the
// compiler can still see straight-line `a*b + c` and fuse it (codex's
// repro: `FMADDD` in the arm64 assembly for exactly this shape).
// Syntactically, the MUL's immediate parent there is the CallExpr, not the
// +/- -- so a check that only looks one level up misses it. This flags one
// level further: MUL -> non-float64/float32 CallExpr -> +/- or compound
// assignment.
//
...
func directOperandOfUnguardedAddSub(info *types.Info, parent map[ast.Node]ast.Node, mul ast.Node) bool {
	...
	if isAddSubBinary(p) || isCompoundAddSubAssign(p) {
		return true
	}
	...
	pp := nonParenParent(parent, call)
	return isAddSubBinary(pp) || isCompoundAddSubAssign(pp)
}
...
// isCompoundAddSubAssign reports whether n is a `+=`/`-=` compound
// assignment -- the implicit-addition shape a plain isAddSubBinary check
// cannot see, since no *ast.BinaryExpr* for the `+`/`-` exists in the AST
// for a compound assignment (see directOperandOfUnguardedAddSub's doc
// comment).
func isCompoundAddSubAssign(n ast.Node) bool {
	assign, ok := n.(*ast.AssignStmt)
	return ok && (assign.Tok == token.ADD_ASSIGN || assign.Tok == token.SUB_ASSIGN)
}
...
//
// Codex round 2 (P1, EXECUTED) found an earlier version of this check was
