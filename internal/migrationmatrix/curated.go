...
var RemainingExecutorLedger = map[string]RemainingCitationRow{
	"capacity":              {Citation: "Go: `internal/jobs/metrics/remaining/capacity_native.go`, `capacity_native_clickhouse.go`", Route: "river, native (`cmd/dev-health-worker/daily.go:477-515`)", Ticket: "CUT-20 R2 (Done)"},
	"complexity":            {Citation: "Go: `internal/jobs/metrics/remaining/complexity_native.go`, `complexity_native_clickhouse.go`", Route: "river, native (`cmd/dev-health-worker/daily.go:517-558`)", Ticket: "CHAOS-4291 (Done)"},
	"dora":                  {Citation: "Go: `internal/jobs/metrics/remaining/dora_native.go`, `dora_native_clickhouse.go`", Route: "river, native (`cmd/dev-health-worker/daily.go:415-475`)", Ticket: "CHAOS-3092 R1 (Done)"},
	"membership_backfill":   {Citation: "Go: `internal/jobs/metrics/remaining/membership_native.go`", Route: "river, native (`cmd/dev-health-worker/daily.go:608-654`)", Ticket: "CHAOS-4282 (Done)"},
	"recommendations":       {Citation: "Go: `internal/jobs/metrics/remaining/recommendations_native.go`", Route: "river, native (`cmd/dev-health-worker/daily.go:560-606`)", Ticket: "CHAOS-4281/CHAOS-3092 (Done)"},
	"release_impact":        {Citation: "Go: `internal/jobs/metrics/remaining/release_impact_native_executor.go`, `release_impact_native_clickhouse.go`. CHAOS-5244: Python daily-compute orchestrator (`job_release_impact.py`, `compute_release_impact_daily`) deleted -- job compute deleted; `release_impact.py`'s `_compute_day` survives only as `fixtures/runner.py`'s local/CI fixture-generation dependency, fixture-generation path pending CHAOS-5250", Route: "river, native (`cmd/dev-health-worker/daily.go:707-756`)", Ticket: "CHAOS-4296 (Done)"},
	"work_item_attribution": {Citation: "Go: `internal/jobs/metrics/remaining/work_item_attribution_native.go` -- CHAOS-3092 PR-B staleness-window backstop, NOT the full daily attribution compute (that's §2's `work_item_attribution` row, native as of CHAOS-5078)", Route: "river, native (`cmd/dev-health-worker/daily.go:656-705`)", Ticket: "CHAOS-3092 PR-B (Done)"},
}
