...
This module has no execution path: query-api serves the GraphQL capacity
operations natively. It is the kernel, retained deliberately as the parity
oracle, on the same precedent as ``metrics/compounding_risk.py``: a Go port
whose Python original is gone cannot be proved to agree with anything, and its
goldens become frozen blobs that only restate whatever the Go code already
does.
...
independent backlog reader against a live ClickHouse. The module defines no other loader: nothing in the tree references one.
