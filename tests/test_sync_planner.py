...
def test_manual_backfill_with_zero_sources_and_discovery_disabled_plans_zero_units(
    db_session, monkeypatch: pytest.MonkeyPatch
):
    """CHAOS-4602 baseline (red): a config whose integration has NEVER had
    any ``integration_sources`` row -- jira's exact situation today
    (``discover_repos_for_config`` has no jira branch yet, CHAOS-4584 still
    in flight) -- plans zero units on a manual/backfill trigger when
    discovery finds nothing, same as before this ticket. Documents the
    starting condition the next test fixes; discovery finding nothing here
    is not a new gap.
    """
    integration = _create_integration(db_session)
    _create_dataset(db_session, integration, "commits")
    monkeypatch.setattr(
        planner, "discover_sources_for_integration", lambda *_a, **_k: []
    )
...
def test_manual_backfill_with_zero_sources_runs_source_discovery_before_unit_planning(
    db_session, monkeypatch: pytest.MonkeyPatch, caplog
):
    """CHAOS-4602: ``plan_sync_run`` -- the seam manual "Sync Now", admin API
    backfill, and the operator backfill tool all share -- now runs source
    discovery BEFORE ``_load_enabled_sources``. A config whose integration
    has NEVER had any ``integration_sources`` row plans units on its very
    first manual/backfill trigger, with no prior config-creation-time
    discovery required. This is the case chris reported: a Jira config that
    was never scheduled and never got sources through the admin UI backfill.

    Uses github here (not jira): ``discover_repos_for_config``'s jira branch
    does not exist yet on this branch's base (CHAOS-4584, in flight) -- this
    test proves the SEAM (plan_sync_run calls discovery before planning),
    which is provider-agnostic; jira coverage is a rebase away once #2036
    merges, not a gap in this seam.
    """
    import logging
    ...
    def _fake_discover(session, integration_id, **_kwargs):
        calls.append(str(integration_id))
        return [
            _create_source(session, integration, external_id="full-chaos/dev-health"),
            _create_source(
                session, integration, external_id="full-chaos/dev-health-web"
            ),
        ]

    monkeypatch.setattr(planner, "discover_sources_for_integration", _fake_discover)

    with caplog.at_level(logging.INFO, logger="dev_health_ops.sync.planner"):
        plan = plan_sync_run(
            db_session,
            SyncPlanRequest(
                integration_id=str(integration.id),
    ...
    # Discovery ran exactly once, for this integration, before planning read
    # sources -- proven by the units below actually existing at all.
    assert calls == [str(integration.id)]
    # 2 discovered sources x 1 dataset x 2 backfill windows (14 days / 7-day
    # chunks) -- matches test_backfill_creates_one_unit_per_source_dataset_window's
    # own math one dataset narrower.
    assert plan.total_units == 4
    units = _planned_units(db_session, plan.sync_run_id)
    assert len(units) == 4
    assert {str(unit.source_id) for unit in units} == {
        str(source.id) for source in db_session.query(IntegrationSource).all()
    }

    # Telemetry: provider + integration_id + sources_upserted/created/existing
    # logged, satisfying the CHAOS-4602 telemetry requirement for this seam.
    [record] = [
        r for r in caplog.records if r.message == "sync.planner.source_discovery"
    ]
    assert record.provider == "github"
    assert record.integration_id == str(integration.id)
    assert record.sources_upserted == 2
    assert record.sources_created == 2
    assert record.sources_existing == 0
    assert record.triggered_by == "backfill"


def test_explicit_source_scope_skips_source_discovery(
    db_session, monkeypatch: pytest.MonkeyPatch
):
    """An explicit ``source_ids`` scope (the config is pinned to one already-
    known source) must never trigger discovery -- matches the Go-native
    materializer seam's identical guard."""
    integration = _create_integration(db_session)
    ...
    def _fake_discover(*_args: object, **_kwargs: object) -> list[IntegrationSource]:
        calls.append("called")
        return []
...
def test_source_discovery_failure_does_not_fail_manual_sync(
    db_session, monkeypatch: pytest.MonkeyPatch
):
    """A discovery failure (provider API error, credential resolution
    failure, ...) is logged loud but never fails planning -- fail-open,
    matching the Go-native materializer seam's identical posture. Zero
    sources existed and discovery failed to create any, so the run still
    legitimately plans zero units -- that is a pre-existing, supported
    ...
    integration = _create_integration(db_session)
    _create_dataset(db_session, integration, "commits")
    monkeypatch.setattr(planner, "discover_sources_for_integration", _boom)
