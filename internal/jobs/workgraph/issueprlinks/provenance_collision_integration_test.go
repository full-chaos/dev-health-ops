...
//
// # WHEN THE FIX LANDS, THESE ASSERTIONS FLIP
//
// Marked FLIP below. Today they assert the fallback survives; after the
// migration they must assert native (Key A) and explicit_text (Key B) survive
// the merge. A migration that leaves them unchanged has not fixed anything.
func TestIssuePRProvenanceCollisionSurvivesMerge(t *testing.T) {
	if os.Getenv("DEV_HEALTH_LIVE_PYTHON_ORACLES") != "1" {
		t.Skip("container-backed acceptance test; run through ci/check_go.sh")
	}
	...
	assertSurvivor(t, visible, prA, "heuristic")
	assertSurvivor(t, visible, prB, "heuristic")
	...
	assertOnlyProvenance(ctx, t, conn, keyA, "heuristic")
	assertOnlyProvenance(ctx, t, conn, keyB, "heuristic")
	...
	t.Run("exact version tie resolves by part recency", func(t *testing.T) {
		...
		for trial := 1; trial <= 10; trial++ {
			...
			if want := order[len(order)-1]; survivor != want {
				t.Errorf("trial %d (%v): survivor = %q, want %q (last-inserted part)",
					trial, order, survivor, want)
			}
