...
func TestProofRouteIsNotRegisteredByDefault(t *testing.T) {
	...
	output := captureLog(t, func() {
		mountProofRoute(os.Getenv, mux, func(http.ResponseWriter, *http.Request) {})
	})
...
func TestProofRouteRegistersOnExplicitOptIn(t *testing.T) {
	...
	mux := http.NewServeMux()
	output := captureLog(t, func() {
		mountProofRoute(os.Getenv, mux, func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusTeapot) })
	})
...
func TestProofRouteRefusesANilHandler(t *testing.T) {
	...
	mux := http.NewServeMux()
	output := captureLog(t, func() { mountProofRoute(os.Getenv, mux, nil) })
	if mountedPaths(t, mux, "/query/proof") {
...
func TestTheProofRouteMountsWithTheRunningBuild(t *testing.T) {
	...
	mux := http.NewServeMux()
	served := false
	mountProofRoute(os.Getenv, mux, func(w http.ResponseWriter, _ *http.Request) {
		served = true
		w.WriteHeader(http.StatusOK)
	})
