...
#: enablement): the identical function used to live in this module as
#: ``_current_schema_digest``. It moved to
#: :mod:`dev_health_ops.api.graphql.go_api_schema_digest` -- unchanged in
#: algorithm, caching and failure behaviour -- so that the ``dev-hops
#: go-api routing`` CLI and ``ci/check_go_api_routing_digest.py`` can
#: import it WITHOUT dragging in this module's httpx/starlette/strawberry
#: imports (a bare interpreter cannot import this file at all). Keeping
#: one producer for the value that both WRITES and READS
#: ``go_api_routing_state``'s primary key is the whole point -- see that
#: module's docstring for why a second implementation would be a silent
#: outage.
#:
#: The alias below is retained because it is the name the dispatcher's own
#: tests monkeypatch and call
#: (``tests/api/graphql/test_go_api_dispatcher.py``); it is the same
#: object, not a wrapper, so ``lru_cache`` state is shared rather than
#: duplicated.
_current_schema_digest = current_schema_digest
