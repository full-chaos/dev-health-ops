...
go-worker` execs ``/usr/local/bin/dho`` inside the running
...
``stat /usr/local/bin/dho: no such file or directory`` --
...
DHO_SOURCE = ROOT / "cmd" / "dho"

_STAGE_HEADER = re.compile(r"^FROM\s+.+\s+AS\s+(?P<name>\S+)\s*$", re.MULTILINE)
_CP_DHO_WORKER = re.compile(r"cp\s+/out/dho\s+(?P<dest>/runtime/worker/\S+);")
...
def _packaged_worker_image_path() -> str:
    """The absolute path dho lands at inside the `worker`
    ...
    cp_match = _CP_DHO_WORKER.search(_build_stage_body())
    assert cp_match is not None, (
        "docker/go-worker.Dockerfile's build stage no longer stages "
        "dho for the worker image; operators have nothing to exec"
    )
    ...
    assert dest.startswith(prefix), (
        f"dho is staged at {dest!r}, which the worker "
...
def test_dho_source_exists() -> None:
    assert DHO_SOURCE.is_dir(), "cmd/dho is missing; there is nothing to package"


def test_the_worker_image_packages_dho_at_a_real_path() -> None:
    assert _packaged_worker_image_path() == "/usr/local/bin/dho"
...
def test_the_container_smoke_gate_execs_the_exact_path_the_image_packages() -> None:
    ...
    assert match is not None, (
        "ci/check_go_containers.sh no longer execs dho as "
