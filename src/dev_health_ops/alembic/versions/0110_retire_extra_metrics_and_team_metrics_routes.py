...
``upgrade()`` queries ``river.river_job`` for any row (any state) whose
``kind`` is one of the two retired kinds and refuses to proceed if it finds
one -- a River job row surviving in that table would mean something DID
enqueue the kind at some point, which would falsify this migration's entire
premise. The check is a no-op (not an error) when the ``river`` schema or
``river_job`` table does not exist yet in the database being migrated --
River owns that schema via its own Go migration tool, not Alembic, and a
database that has never run River's migrations trivially has no such rows.
"""
...
def _assert_no_river_job_rows_for_retired_kinds() -> None:
    ...
    table_exists = bind.execute(
        sa.text("SELECT to_regclass('river.river_job') IS NOT NULL")
    ).scalar_one()
    if not table_exists:
        return
    found = bind.execute(
        sa.text(
            "SELECT kind, count(*) AS n FROM river.river_job "
    ...
    if found:
        ...
        raise RuntimeError(
            "refusing to retire remaining-metrics kinds with existing "
            "river.river_job rows (any state) -- CHAOS-4243's premise is "
