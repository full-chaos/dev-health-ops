...
func peopleDrilldownIssuesSwitchFromEnv(getenv getenvFunc) *routeswitch.DynamicSwitch {
	sw := routeswitch.NewDynamicSwitch()
	if enabled, _ := strconv.ParseBool(getenv(peopleDrilldownIssuesEnabledEnvVar)); enabled {
...
func loadPeopleDrilldownIssuesRouteConfig(getenv getenvFunc) (clickHouseURI, jwksPath, issuer, audience string, ok bool) {
...
// this binary follows.
func buildPeopleDrilldownIssuesRoute(getenv getenvFunc) (handler http.HandlerFunc, cleanup func(), ok bool, err error) {
	clickHouseURI, jwksPath, issuer, audience, cfgOK := loadPeopleDrilldownIssuesRouteConfig(getenv)
	if !cfgOK {
	...
	routeMux := routeswitch.NewMux(peopleDrilldownIssuesSwitchFromEnv(getenv))
