...
async def test_clickhouse_store_context_manager_initializes_and_creates_tables():
    ...
    with (
        ...
        patch("dev_health_ops.storage_impl.Path") as MockPath,
    ):
...
async def mongo_store():
    ...
    with patch("dev_health_ops.storage_impl.AsyncIOMotorClient") as mock_client_class:
...
async def test_mongo_store_init_with_connection_string():
    ...
    with patch("dev_health_ops.storage_impl.AsyncIOMotorClient"):
