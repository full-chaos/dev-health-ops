...
// document which interpreter generated it without describing anything int()
// accepts or rejects: a CPython patch release moves these with no change to
// the Unicode data this guard protects.
//
// int_max_str_digits is deliberately absent from this list. It is a runtime
// interpreter setting that decides which values int() accepts at all, so
// unlike python_version and unidata_version it stays part of the compared
// payload, checked against whatever the live interpreter reports today.
var decimalDigitsProvenanceFields = []string{"python_version", "unidata_version"}

// splitGoldenPayload separates a golden document's PAYLOAD from a fixed set
// of documentation-only fields, so a rot guard can compare the payload alone
// and still report which of the excluded fields moved on a mismatch.
//
// The payload is re-marshalled rather than sliced out of the original bytes,
// which also makes the comparison insensitive to key order and formatting --
// the concern is what the document SAYS, not how it happens to be rendered.
func splitGoldenPayload(t *testing.T, document []byte, fields []string, label string) (payload []byte, extracted map[string]string) {
	t.Helper()
	...
	for _, field := range fields {
		...
		if err := json.Unmarshal(raw, &value); err != nil {
			t.Fatalf("decode %s golden field %q: %v", label, field, err)
		}
	...
	if err != nil {
		t.Fatalf("re-serialise %s golden payload: %v", label, err)
	}
...
// formatFields renders a documentation-field map with stable key order, so a
// mismatch message is deterministic rather than a function of map iteration.
...
// decimalDigitsGoldenVerdict is the guard's pass/fail decision for a pair of
// decimal-digits golden documents, kept separate from *testing.T so it can be
// exercised directly against synthetic documents.
type decimalDigitsGoldenVerdict struct {
	Pass    bool
	Message string
}

// evaluateDecimalDigitsGolden is the decimal-digits guard's comparison.
//
// It compares the PAYLOAD only: python_version and unidata_version are
// documentation about which interpreter produced the corpus, not part of what
// the corpus asserts, and a difference confined to them is reported -- to say
// which side moved -- without failing the guard. int_max_str_digits and
// decimal_digits stay in the payload and gate the verdict, because the first
// decides which values int() accepts at all and the second is what the guard
// exists to protect.
func evaluateDecimalDigitsGolden(t *testing.T, frozen, rendered []byte) decimalDigitsGoldenVerdict {
	t.Helper()
	frozenPayload, frozenFields := splitGoldenPayload(t, frozen, decimalDigitsProvenanceFields, "committed")
	renderedPayload, renderedFields := splitGoldenPayload(t, rendered, decimalDigitsProvenanceFields, "regenerated")

	if string(frozenPayload) == string(renderedPayload) {
		if !equalStringMaps(frozenFields, renderedFields) {
			return decimalDigitsGoldenVerdict{Pass: true, Message: fmt.Sprintf(
				"decimal-digits golden payload matches; documentation-only fields moved\n"+
					"  committed: %s\n  live:      %s",
				formatFields(frozenFields), formatFields(renderedFields),
			)}
		}
		return decimalDigitsGoldenVerdict{Pass: true, Message: "decimal-digits golden matches the live interpreter"}
	}
	...
	if skew := describeInterpreterSkew(frozen, rendered); skew != "" {
		return decimalDigitsGoldenVerdict{Pass: false, Message: fmt.Sprintf(
	...
	return decimalDigitsGoldenVerdict{Pass: false, Message: "live Python no longer reproduces the frozen decimal-digits golden, and " +
...
// TestDecimalDigitsGoldenPayloadOnlyComparison pins the guard's verdict on two
// synthetic document pairs, without needing a live interpreter: one that
// differs only in a documentation field, and one that differs in the runtime
// digit-limit setting.
func TestDecimalDigitsGoldenPayloadOnlyComparison(t *testing.T) {
	base := map[string]any{
		"python_version":     "3.14.7",
		"unidata_version":    "16.0.0",
		"int_max_str_digits": 4300,
		"decimal_digits":     [][]int{{48, 0}, {49, 1}},
	}

	t.Run("documentation field alone differs", func(t *testing.T) {
		frozen := marshalGoldenDocument(t, base)
		live := cloneGoldenDocument(base)
		live["python_version"] = "3.14.8"
		rendered := marshalGoldenDocument(t, live)

		verdict := evaluateDecimalDigitsGolden(t, frozen, rendered)
		if !verdict.Pass {
			t.Fatalf("payload is identical and only python_version moved; want PASS, got FAIL: %s", verdict.Message)
		}
	})

	t.Run("behaviour-affecting field differs", func(t *testing.T) {
		frozen := marshalGoldenDocument(t, base)
		live := cloneGoldenDocument(base)
		live["int_max_str_digits"] = 4301
		rendered := marshalGoldenDocument(t, live)

		verdict := evaluateDecimalDigitsGolden(t, frozen, rendered)
		if verdict.Pass {
			t.Fatal("int_max_str_digits moved; want FAIL, it decides which values int() accepts")
		}
	})
}

func cloneGoldenDocument(source map[string]any) map[string]any {
...
func marshalGoldenDocument(t *testing.T, fields map[string]any) []byte {
	t.Helper()
	encoded, err := json.Marshal(fields)
	if err != nil {
		t.Fatalf("marshal synthetic golden document: %v", err)
	}
	return encoded
}
