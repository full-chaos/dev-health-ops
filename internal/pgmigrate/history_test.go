...
func TestHistoryWithAChain(t *testing.T) {
	...
	chain := []ChainFile{
		{Revision: "0143", Name: "0143_add_widget_table.sql"},
		{Revision: "0144", Name: "0144_widget_index.sql"},
	}
	...
	if top[0] != "0143 -> 0144 (head), Widget index." || top[1] != application+" -> 0143, Add widget table." {
		t.Errorf("chain lines = %q", top[:2])
	}
	if !strings.HasPrefix(top[2], "0141 -> "+application+" (application_schema), ") || strings.Contains(top[2], "(head)") {
