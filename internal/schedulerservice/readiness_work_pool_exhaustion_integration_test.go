...
func TestSchedulerReadinessChecksDoNotQueueBehindTheQueueAndCoordinatorWorkPools(t *testing.T) {
	...
	newWorkPool := func() *pgxpool.Pool {
		...
		config.MaxConns = 2 // the scheduler's queue_control and coordinator sizes
	...
	database := &postgresSchedulerDatabase{
