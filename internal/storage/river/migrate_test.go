...
func TestRuntimeGrantStatementsMatchProvisionedLeastPrivilegePolicy(t *testing.T) {
	...
	want := []string{
		...
		"DO $$ BEGIN IF to_regclass('public.scheduled_sync_occurrences') IS NOT NULL THEN GRANT SELECT, INSERT, UPDATE ON TABLE public.scheduled_sync_occurrences TO \"domain_runtime\"; END IF; END $$",
		"DO $$ BEGIN IF to_regclass('public.fixed_schedule_occurrences') IS NOT NULL THEN GRANT SELECT, INSERT, UPDATE ON TABLE public.fixed_schedule_occurrences TO \"domain_runtime\"; END IF; END $$",
