...
// the operator tools image, relative to that image's working directory
// (docker/go-api-tools.Dockerfile writes it at /app/go-api/documents.json
// and sets WORKDIR /app/go-api). It is a default rather than a constant
// reached directly, for the reason DefaultCatalogPath is: a binary run
// from somewhere else must be able to say where the file is, and a wrong
// path must be a refusal rather than a silently empty registry.
//
// The dump is generated by registrydump AT IMAGE BUILD TIME from the
// same commit as the binaries beside it, so what it describes is exactly
// what the image about to roll will register.
const DefaultDocumentsPath = "documents.json"
