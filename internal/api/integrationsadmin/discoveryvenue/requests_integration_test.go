...
VALUES ($1, 'disc-noorg@example.com', true, true, false, 0, $2, $2)`, v.adminNoOrg, at)
	...
	for _, mapping := range []map[string]any{
		...
		{"email": "venue@example.com", "api_token": "forbidden-token", "base_url": jiraURL},
...
VALUES ($1, $2, 'jira', $3, true, $4, '{}'::json, $5, $5)`, id, org.String(), name, ciphertext, at)
	...
	credential(v.credForbidden, v.orgA, "jira forbidden", ciphertexts[2])
...
// authFailureRequests are discoveries the provider answers with 401 and 403
// (Jira, whose Python discovery raises: the fixed 503): the Go api answers 422.
func authFailureRequests(venue *venueoracle.Venue, v ids) []venueoracle.Request {
	bearer := map[string]string{"Authorization": "Bearer " + venue.Tokens["adminA"]}
	return []venueoracle.Request{
		{Name: "provider refuses the credential (401)", Method: "POST", Path: discoverPath + v.intBad.String() + "/discover", Headers: bearer},
		{Name: "provider forbids the credential (403)", Method: "POST", Path: discoverPath + v.intForbidden.String() + "/discover", Headers: bearer},
	}
}
