...
// TestOrgAndUserCRUDMatchesThePythonAPI is the venue-oracle proof for the
// remaining CHAOS-6250 routes: org list/get/create/patch, member
// list/add/patch-role/remove, transfer-ownership, and user
// list/get/patch/delete. DELETE /orgs/{org_id} is out of scope (stubbed
// 501, see orgs.go's deleteOrganizationStub doc comment).
func TestOrgAndUserCRUDMatchesThePythonAPI(t *testing.T) {
	ctx := context.Background()
	root := repoRoot(t)
	const jwtKey = "venue-oracle-test-secret-key-for-org-user-crud-32-bytes!!"
...
VALUES ($1, $2, $3, 'member', now(), now(), now())`, uuid.New(), orgID, newMemberID)
	...
	requests := []venueoracle.Request{
		...
		// Users.
		{Name: "list users by org", Method: "GET", Path: "/api/v1/admin/users", Headers: map[string]string{"Authorization": bearer("owner"), "X-Org-Id": orgID.String()}},
		{Name: "get user", Method: "GET", Path: "/api/v1/admin/users/" + memberID.String(), Headers: authHeaders("owner")},
		{Name: "get user not found", Method: "GET", Path: "/api/v1/admin/users/" + uuid.New().String(), Headers: authHeaders("owner")},
		{Name: "get user out of scope", Method: "GET", Path: "/api/v1/admin/users/" + superID.String(), Headers: authHeaders("owner")},
		{Name: "patch user", Method: "PATCH", Path: "/api/v1/admin/users/" + memberID.String(), Headers: jsonHeaders("owner"),
			Body: venueoracle.B64(`{"full_name":"A New Name"}`)},
		{Name: "delete user", Method: "DELETE", Path: "/api/v1/admin/users/" + memberID.String(), Headers: authHeaders("owner")},
		{Name: "delete user again", Method: "DELETE", Path: "/api/v1/admin/users/" + memberID.String(), Headers: authHeaders("owner")},
	}
