...
BRIDGE_ROUTE_LEDGER: dict[str, dict[str, str]] = {
    ...
    "/api/internal/worker-sync/reference-discovery-populate": {
        ...
        "computes": "jira teams/members/memberships/project-ownership rows via this route (the only provider whose call actually writes); credential decrypt + PagerDuty OAuth rotation Python-side. linear/github/gitlab sync runs never reach this handler at all (Go never calls it for them). CHAOS-4498: backfill's arm-and-wait now reaches this route the same way sync-time discovery does -- it is no longer a separate direct-call code path (see `sync.team_autoimport` kind row's writer column)",
        "state": "bridge — live for jira sync-time AND backfill dispatch; dead for linear/github/gitlab both dispatch modes pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven. CHAOS-4498 (this PR): the backfill tool no longer calls the underlying Python function directly -- it reaches this route (or bypasses it, for a native provider) through the SAME `TeamCatalogDiscoveryExecutor` seam sync-time dispatch uses",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native — the backfill blocker CHAOS-4498 closed) + CHAOS-4492 (ledger regen) + CHAOS-4498 (backfill re-pointed, this PR)",
    },
    "/api/internal/worker-sync/team-autoimport": {
        ...
        "python_handler": "`worker_sync.py:269 team_autoimport_reference` -> `team_autoimport.py:228 run_post_sync_team_autoimport` -> same per-provider `populate()`. jira is the only provider reaching this route with a real populator; pagerduty/launchdarkly reach it too but `_provider_capability` is false for them, so it no-ops",
        "computes": "same tables as reference-discovery-populate, best-effort variant, jira sync-time dispatch only (the only provider reaching this route whose call actually writes)",
        "state": "bridge — live for jira sync-time AND backfill dispatch; dead for linear/github/gitlab both dispatch modes pending 5.6 prod readback (CHAOS-4492) confirming the native collectors are the live prod writer, not just local/CI-proven. CHAOS-4498 (this PR): the backfill tool no longer calls the underlying Python function directly -- it reaches this route (or bypasses it, for a native provider) through the SAME `TeamCatalogDiscoveryExecutor` seam sync-time dispatch uses",
        "ticket": "CHAOS-4198 (jira, remaining scope) + CHAOS-4435 (retirement once jira native — the backfill blocker CHAOS-4498 closed) + CHAOS-4492 (ledger regen) + CHAOS-4498 (backfill re-pointed, this PR)",
    },
}
