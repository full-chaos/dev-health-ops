...
6. PagerDuty only: `hydrate_pagerduty_credentials` runs after resolution. In `oauth` auth mode it reads OAuth storage, validates scopes and bindings and may refresh a token (a WRITE); failure raises: no estimate. The budget estimator itself needs only `region` and `subdomain`, never the token.
...
7. Named exception E1 (PagerDuty OAuth hydration): Python computes no estimate when `hydrate_pagerduty_credentials` fails in `oauth` mode (missing OAuth app config, scope or binding error, refresh failure). Go cannot reproduce that without reading OAuth storage and possibly refreshing (a write), which an estimator must not do. Go returns the estimate anyway (constraint applied where Python applied none). Harm bound: the pagerduty bucket has no configured limit (default 1,000,000), so no defer is possible at current settings; shadow class `go_extra_estimate` on `auth_mode=oauth` units is expected and excluded from the adoption bar; the switch PR for pagerduty states the limit in force. Overturnable: replicate hydration failure by a read-only check.
