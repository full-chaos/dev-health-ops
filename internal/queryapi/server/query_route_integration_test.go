...
const (
	// itTestSchemaDigest is an ARBITRARY test value -- every call site
	// below passes it straight to newQueryHandler (routing-key plumbing
	// only; buildQueryRoute no longer takes or verifies a schema digest at
	// all, CHAOS-5013), so an arbitrary value is safe here.
...
func TestBuildQueryRoute_FailsFastOnWrongClickHouseProtocol(t *testing.T) {
	...
	_, _, _, err := buildQueryRoute(os.Getenv, cfg)
	if err == nil {
