...
func newBuildInfoHandler(verifier *principal.Verifier) http.HandlerFunc {
	...
	if err != nil {
		...
		panic(fmt.Sprintf("query-api: /buildinfo response could not be encoded: %v", err))
	}
...
func mountProofRoute(getenv getenvFunc, mux *http.ServeMux, handler http.HandlerFunc) {
	posture := strings.ToLower(strings.TrimSpace(getenv(deploymentEnvEnv)))
	enabled := strings.EqualFold(strings.TrimSpace(getenv(proofRouteEnabledEnv)), "true")
