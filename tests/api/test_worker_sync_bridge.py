...
def test_team_auto_import_reference_is_identifiers_only() -> None:
    """Pins the request payload shape so nobody later "optimizes" secret
    material into it. TeamAutoImportReference (reused by
    /dispatch-budget-estimate's tenant check) must declare EXACTLY
    organization_id and sync_run_id, and forbid any extra field -- a
    client-supplied "credentials" key must be rejected at the schema layer,
