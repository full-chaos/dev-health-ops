...
func TestRunbookInvocationUsesRealFlags(t *testing.T) {
	documented := documentedInvocations(t, "dho goapi prove")
	if len(documented) == 0 {
		t.Fatal("docs carry no dho goapi prove invocation")
	}
	...
	for _, invocation := range documented {
		...
		if err := fs.Parse(invocation.args); err != nil {
			t.Errorf("%s: the documented %s command does not parse: %v", invocation.doc, "dho goapi prove", err)
		}
		for _, name := range invocation.flags {
			if fs.Lookup(name) == nil {
				t.Errorf("%s documents -%s, which dho goapi prove does not define", invocation.doc, name)
			}
		}
	}
}
