...
func TestRunbookInvocationUsesRealFlags(t *testing.T) {
	documented := documentedInvocations(t, "dho goapi rest-prove")
	if len(documented) == 0 {
		t.Fatal("docs carry no dho goapi rest-prove invocation")
	}

	for _, invocation := range documented {
		// A fresh flag set per invocation, same shape parseFlags builds:
		// an unknown flag or a value its type rejects fails here.
		fs, _ := registerFlags()
		fs.SetOutput(io.Discard)
		if err := fs.Parse(invocation.args); err != nil {
			t.Errorf("%s: the documented %s command does not parse: %v", invocation.doc, "dho goapi rest-prove", err)
		}
		for _, name := range invocation.flags {
			if fs.Lookup(name) == nil {
				t.Errorf("%s documents -%s, which dho goapi rest-prove does not define", invocation.doc, name)
			}
		}
	}
}
...
func documentedInvocations(t *testing.T, binary string) []documentedInvocation {
	...
	err := filepath.WalkDir("../../../docs", func(path string, entry fs.DirEntry, err error) error {
		...
		for _, command := range runbookCommands(string(raw), binary) {
			...
			out = append(out, documentedInvocation{doc: path, flags: flags, args: runbookArgv(command)[len(strings.Fields(binary)):]})
		}
		return nil
	})
	if err != nil {
