...
operator kill switch. ``ask_dev`` is registered as an explicit-enable feature,
so every tier remains denied until an organization or license override grants
it. The migration intentionally does not touch ``byo_llm`` or
...
_FEATURE_KEY = "ask_dev"
_FEATURE_NAME = "Ask Dev"
...
def downgrade() -> None:
    # Entitlement rows may be referenced by org overrides. Keep the additive
    # registration on code rollback, matching the existing ACR entitlement.
    pass
