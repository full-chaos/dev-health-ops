...
func TestBaselineLoads(t *testing.T) {
	...
	if !baseline.Cutover || baseline.RiverSchema != "river" || !reflect.DeepEqual(baseline.Heads, []string{"0066", "0139"}) {
		t.Fatalf("baseline settings cutover=%v schema=%q heads %v; want production's (true, river, [0066 0139])",
